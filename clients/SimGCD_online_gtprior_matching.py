...
class SimGCDOnlineGTPriorMatchingClients():
    ...
    def setup(self, model, device, local_dataset, global_epoch, local_lr, trainer, **kwargs):
        ...
        if self.args.client.sampler == 'weighted':
            ...
            sampler = RandomMultipleGallerySamplerNoCam(local_dataset, self.args.num_instances)
        ...
        print(f'Global Epoch {global_epoch+1} Learning Rate: {local_lr}')
    ...
    def local_train(self, global_epoch, **kwargs):
        ...
        for local_epoch in range(self.args.trainer.local_epochs):
            ...
            for i, (images, labels, uq_idxs, mask_lab) in tqdm(enumerate(self.loader), total=len(self.loader)):
                ...
                online_prior = online_prior / len(labels_unlabelled)  # Normalize to get probability 

                # class_weights = torch.ones_like(labels, dtype=torch.float).to(self.device)
                # online_prior.index_add_(0, labels, class_weights)
                # online_prior = online_prior / len(labels)  # Normalize to get probability distribution
    ...
    def _algorithm(self, images, labels, uq_idxs, mask_lab, global_epoch=0, local_epoch=0, online_prior=None) -> Dict:
        ...
        ### only for debuggging
        student_out =  torch.cat([f[~mask_lab] for f in (student_out).chunk(2)], dim=0)
