...
class Interpolater():
    ...
    def get_interpolate_model(self, factor, stochastic=False):
        ...
        # branch_index = torch.randint(3, (num_layers,))
    ...
    def get_interpolate_state_dict(self, factor, stochastic=False):
        # Note: it will change self.inter_model
        inter_state_dict = self.inter_model.state_dict()
        global_state_dict = self.global_model.state_dict()
        local_state_dict = self.local_model.state_dict()
        ...
        # self.inter_model.load_state_dict(inter_state_dict)
        # stoc_inter_results = self.trainer.evaler.eval(model=inter_model, epoch=global_epoch)
        return inter_state_dict
    

    # interpolate into model1
    def interpolate_models(self, model1, model2, factor): 
        model1_state_dict = model1.state_dict()
        model2_state_dict = model2.state_dict()

        for key in model1_state_dict.keys():
            model1_state_dict[key] = model1_state_dict[key] + factor * (model2_state_dict[key] - model1_state_dict[key])
    ...
    def update(self, progress=0):
        ...
        with torch.no_grad():
            ...
            if self.inter_args.type == "noise":
                ...
                elif self.inter_args.type == "progress":
                    start, end = self.inter_args.progress_interval
    ...
    def forward_stoc(self, x: torch.Tensor, repeat: int, no_relu: bool, ):
        ...
        for m in range(repeat):
            ...
            results['layer5'] = results['logit']
            if no_relu == True:
                results['feature'] = F.relu(results['layer4'])
    ...
    #TODO
    def forward_inter_mixup(self, x: torch.Tensor, no_relu: bool):
        return self.inter_model(x, no_relu=no_relu)
    ...
    def forward(self, x: torch.Tensor,  repeat: int = 1, no_relu: bool = False) -> Dict:
        ...
        elif self.inter_args.forward_type == "layer_returnfeat":
            stoc_outs = self.forward_stoc(x, repeat=repeat, no_relu=no_relu)
            #assert repeat==1
            assert(len(stoc_outs)==1)
            local_out = self.forward_local(x, no_relu=no_relu)
            results = {
                "branch_result": stoc_outs[0],
                "local_result": local_out
            }
...
@CLIENT_REGISTRY.register()
class InterpolateSwitchClient(InterpolateClient):
    ...
    def setup(self, model, device, local_dataset, local_lr, global_epoch, trainer, **kwargs):
        ...
        # self.optimizer = optim.SGD(list(self.interpolater.local_model.parameters()) + list(self.interpolater.inter_model.parameters()),
        #                             lr=local_lr, momentum=self.args.optimizer.momentum,
        #                            weight_decay=self.args.optimizer.wd)
    ...
    def local_train(self, global_epoch, **kwargs):
        ...
        for local_epoch in range(self.args.trainer.local_epochs):
            ...
            self.optimizer = optim.SGD(self.interpolater.local_model.parameters(), lr=self.local_lr, momentum=self.args.optimizer.momentum,
                                   weight_decay=self.args.optimizer.wd)
            # self.optimizer = optim.SGD(list(self.interpolater.local_model.parameters()) + list(self.interpolater.inter_model.parameters()),
            #                         lr=self.local_lr, momentum=self.args.optimizer.momentum,
            #                        weight_decay=self.args.optimizer.wd)
            ...
            # switch local and inter models
            # breakpoint()
            if local_epoch % 2 == 0:
                # print((next(self.interpolater.local_model.parameters()))[-1], (next(self.interpolater.inter_model.parameters()))[-1])
                self.interpolater.local_model, self.interpolater.inter_model = self.interpolater.inter_model, self.interpolater.local_model
                # self.interpolater.interpolate_models(self.interpolater.local_model, self.interpolater.inter_model, factor=0.5)
                
                # print((next(self.interpolater.local_model.parameters()))[-1])
                # print("===")
            # self.interpolater.local_model = self.interpolater.inter_model
