...
class SimGCD_Update_Prior_Global_LC_Client():
    ...
    def get_params_groups(self, model, local_lr):
        regularized = []
        proj_layer_regularized = []
        proj_layer_not_regularized = []
        not_regularized = []
        for name, param in model.named_parameters():
            if not param.requires_grad:
                continue
            ...
            if 'last_layer' in name:
                if name.endswith(".bias") or len(param.shape) == 1:
                    proj_layer_not_regularized.append(param)
                else:
                    proj_layer_regularized.append(param)
            elif name.endswith(".bias") or len(param.shape) == 1:
                not_regularized.append(param)
            else:
                regularized.append(param)
        return [{'params': regularized}, {'params': not_regularized, 'weight_decay': 0.}, {'params': proj_layer_regularized, 'lr': self.args.client.lr_decay_proj_layer * local_lr, 'weight_decay': 0.}, {'params': proj_layer_not_regularized, 'lr': self.args.client.lr_decay_proj_layer * local_lr}]
    ...
    def setup(self, model, device, local_dataset, global_epoch, local_lr, trainer, **kwargs):
        ...
        if global_epoch >= self.args.client.start_update and global_epoch >= self.args.trainer.warmup:
            num_clusters, est_prior_current, all_acc, old_acc, new_acc, aligned_w, aligned_preds, ind_map_pred_to_gt = update_prior(self.args, self.prior_dist, copy.deepcopy(self.model),
            ...
            self.prior_dist = est_prior_current
    ...
    def local_train(self, global_epoch, **kwargs):
        ...
        if self.args.client.get_local_centroid and global_epoch >= self.args.server.start_update:
            local_centroids_dict, local_labelled_centroids_dict = self.get_centroids(copy.deepcopy(self.model), copy.deepcopy(self.loader),  self.evaler, ind_map, self.num_clusters, self.device)
        else:
            local_centroids_dict = None
    ...
    def get_centroids(self, model, loader, evaler, ind_map, K, device):

        # get features from model
        feats_labelled, feats_proj_labelled, logits_labelled, targets_labelled, feats_unlabelled, feats_proj_unlabelled, logits_unlabelled, targets_unlabelled, mask = extract_local_features(self.args, model, loader, evaler, device)
        ...
        if self.args.client.local_centroid_type == 'finch':
            feats_unlabelled_np = feats_unlabelled.numpy()
            c, num_clust, req_c = FINCH(feats_unlabelled_np, req_clust=K, distance='cosine', verbose=False)
            req_c = req_c.astype(int)


            D = max(req_c.max(), targets_unlabelled.max()) + 1
            ...
            for cluster_ind in range(K):
                cluster_feats = feats_unlabelled[req_c == cluster_ind]
    ...
    def _algorithm(self, images, labels, uq_idxs, mask_lab, global_epoch=0) -> Dict:
        ...
        if self.args.client.centroids_cls_weight > 0 and global_epoch >= self.args.server.start_update:
            global_centroids = self.model.global_centroids.weight.data.clone().to(self.device)
            global_centroids = F.normalize(global_centroids, dim=1)

            centroids_logits = torch.matmul(student_feat, global_centroids.T)
            centroids_labels = centroids_logits.argmax(dim=1)
            centroids_ce_loss = F.cross_entropy(student_out, centroids_labels)
        else:
            centroids_ce_loss = 0
