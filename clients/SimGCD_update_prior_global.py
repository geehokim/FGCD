...
class SimGCD_Update_Prior_Global_Client():
    ...
    def setup(self, model, device, local_dataset, global_epoch, local_lr, trainer, **kwargs):
        ...
        if global_epoch >= self.args.client.start_update and global_epoch >= self.args.trainer.warmup:
            if self.args.client.update_alg == 'threshold_finch':
                ...
                num_clusters, est_prior_current, all_acc, old_acc, new_acc, aligned_w, aligned_preds, ind_map_pred_to_gt, req_c = update_prior(self.args, self.prior_dist, copy.deepcopy(self.model),
                                            copy.deepcopy(self.loader), self.evaler,
                                            self.num_classes, global_epoch, self.device, K=len(list(sorted_class_dict.keys())))
        ...
        ## Temperature scaling
        # temperature = self.args.client.prior_temperature
        # self.prior_dist = (self.prior_dist ) ** (1 /  temperature)
        # self.prior_dist /= self.prior_dist.sum()
    ...
    def get_weights(self, epoch=None):
        ...
        weights = {
            ...
            "point_wise_ent": self.args.client.point_wise_ent_weight,
        }
    ...
    def local_train(self, global_epoch, **kwargs):
        ...
        # if self.args.client.re_init_prototypes:
        #     if (global_epoch % 200) == 0 and self.args.trainer.num_clients == 1:
        #         new_prototypes = self.classifiier_init_clustering(copy.deepcopy(self.model), copy.deepcopy(self.loader))
        ...
        for local_epoch in range(self.args.trainer.local_epochs):
            ...
            if self.args.client.train_classifier_only:
                if local_epoch < self.args.client.train_classifier_only_epochs:
                    self.model.freeze_extractor()
    ...
    def _algorithm(self, images, labels, uq_idxs, mask_lab, global_epoch=0) -> Dict:
        ...
        # avg_probs = (student_out / 0.04).softmax(dim=1).mean(dim=0)
        ...
        elif self.args.client.distance_type == 'kl_rescale':
            avg_probs = avg_probs ** (1 / self.args.client.prior_temperature)
            avg_probs = avg_probs / avg_probs.sum()
            avg_probs = avg_probs + 1e-7
            prior_dist = self.prior_dist + 1e-7
            entropy_distance = torch.sum(prior_dist * torch.log(prior_dist / avg_probs))

        elif self.args.client.distance_type == 'kl_rescale_no_norm':
            avg_probs = avg_probs ** (1 / self.args.client.prior_temperature)
            avg_probs = avg_probs + 1e-7
            prior_dist = self.prior_dist + 1e-7
            entropy_distance = torch.sum(prior_dist * torch.log(prior_dist / avg_probs))
        elif self.args.client.distance_type == 'kl_hard_negatives_only':
            avg_probs += 1e-8
            prior_dist = self.prior_dist + 1e-8
            kl_divergence = prior_dist * torch.log(prior_dist / avg_probs)
            k = self.args.client.top_k
            top_k_kl_divergences, _ = torch.topk(kl_divergence, k)
            entropy_distance = torch.sum(top_k_kl_divergences)        

        elif self.args.client.distance_type == 'kl_regularized':
            avg_probs += 1e-8
            prior_dist = self.prior_dist + 1e-8
            entropy_distance = torch.sum(prior_dist * torch.log(prior_dist / avg_probs))
            entropy_distance += 0.5 * self.args.client.kl_lambda * torch.sum((avg_probs - prior_dist) ** 2)
            # entropy_distance = torch.sum( avg_probs * torch.log(avg_probs /prior_dist))
        elif self.args.client.distance_type == "tsallis":
            avg_probs += 1e-8
            prior_dist = self.prior_dist + 1e-8
            q = self.args.client.tsallis_q
            entropy_distance =  (q - 1) ** -1 * (torch.sum(avg_probs ** q - prior_dist ** q - prior_dist ** (q-1)* (avg_probs - prior_dist)))
        ...
        elif self.args.client.distance_type == 'kl_ema':
            avg_probs = torch.clamp(avg_probs, 1e-8, 1)
            prior_dist = torch.clamp(self.prior_dist, 1e-8, 1)
            if self.prior_bank is None:
                self.prior_bank = avg_probs
            else:
                self.prior_bank = self.prior_bank.clone().detach() * self.args.client.ema_decay + avg_probs * (1 - self.args.client.ema_decay)
            self.prior_bank = torch.clamp(self.prior_bank, 1e-8, 1)
            print(self.prior_bank.mean())
            entropy_distance = torch.sum(prior_dist * torch.log(prior_dist / self.prior_bank))
        ...
        # Minimize point-wise entropy

        # Class association loss using global centroids
        if self.args.client.centroids_cls_weight > 0 and global_epoch >= self.args.client.start_update + 1:
            global_centroids = self.model.global_centroids.weight.data.cpu().clone().detach().to(self.device)
            global_centroids = F.normalize(global_centroids, dim=1)

            centroids_logits = torch.matmul(student_feat, global_centroids.T)
            centroids_labels = centroids_logits.clone().detach().to(self.device).argmax(dim=1)
            centroids_ce_loss = F.cross_entropy(centroids_logits / 0.1, centroids_labels)
            losses["centroids_cls"] = centroids_ce_loss
        else:
            centroids_ce_loss = 0

        if self.args.client.point_wise_ent_weight > 0:
            
            logits  = (student_out / 0.1).softmax(dim=1)
            # point_wise_entropy
            point_wise_entropy = - torch.sum(logits * torch.log(logits + 1e-7), dim=1)
            point_wise_entropy_loss = torch.mean(point_wise_entropy)
            losses["point_wise_ent"] = point_wise_entropy_loss
