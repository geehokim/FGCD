...
import random
import numpy as np
from utils.train_utils import update_prior, update_prior_threshold, update_prior_with_novel_centroids
...
class SimGCD_Update_Prior_Local_Client():
    ...
    def local_train(self, global_epoch, **kwargs):
        ...
        print(f"Prior dist: {self.prior_dist}")
        ...
        #novel_class_mask = self.get_novel_class_mask()
        # if global_epoch >= self.args.client.start_update and global_epoch >= self.args.trainer.warmup:
            
        #     num_clusters, est_prior_current, all_acc, old_acc, new_acc, aligned_w, aligned_preds, ind_map_pred_to_gt, cluster_centroids, cluster_targets = update_prior_with_novel_centroids(self.args, self.prior_dist, copy.deepcopy(self.model),
        #                                     copy.deepcopy(self.loader), self.evaler,
        #                                     self.num_classes, global_epoch, self.device, K=len(list(self.sorted_class_dict.keys())))
        ...
        if self.args.client.get_local_centroid:
            cluster_centroids, cluster_targets = self.get_centroids(copy.deepcopy(self.model), copy.deepcopy(self.loader),  self.evaler, self.num_clusters, global_epoch, self.device)
        else:
        ...
        results = {
                'cluster_means': cluster_centroids,
    ...
    def get_centroids(self, model, loader, evaler, K, global_epoch, device):
        ...
        if self.args.client.use_gt_prior:
            for novel_class in range(len(self.args.dataset.seen_classes), self.num_classes):

                novel_class_indices = (targets_unlabelled == novel_class).nonzero(as_tuple=True)[0]
                if len(novel_class_indices) > 0:
                    novel_class_prototype = feats_unlabelled[novel_class_indices].mean(dim=0)
                    cluster_means.append(novel_class_prototype)
                    cluster_targets.append(novel_class)
                    
            cluster_means = torch.stack(cluster_means)
            cluster_targets = torch.LongTensor(cluster_targets)        
        else:

            num_clusters, est_prior_current, all_acc, old_acc, new_acc, aligned_w, aligned_preds, ind_map_pred_to_gt, cluster_means, cluster_targets = update_prior_with_novel_centroids(self.args, copy.deepcopy(self.prior_dist), copy.deepcopy(self.model),
