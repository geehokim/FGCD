...
class FeatureClusteringClient():
    ...
    def evaluate_local_trainset(self, epoch: int, local_datasets: List[torch.utils.data.Dataset] = None) -> Dict:
    ...
    def get_weights(self, epoch=None):
        ...
        weights = {
            "self_cls": 1 - self.args.client.sup_con_weight,
            "sup_cls": self.args.client.sup_con_weight,
            "sup_con": self.args.client.sup_con_weight,
            "con": 1 - self.args.client.sup_con_weight,
            'feat_cluster': 1 - self.args.client.sup_con_weight,
        }
    ...
    def local_train(self, global_epoch, **kwargs):
        ...
        for local_epoch in range(self.args.trainer.local_epochs):
            ...
            if self.args.trainer.local_eval and (local_epoch+ 1) % 5 == 0:
            # if self.args.trainer.local_eval:
                self.evaluate(epoch=local_epoch, local_datasets=None)
                self.evaluate_local_trainset(epoch=local_epoch, local_datasets=None)
    ...
    def clustering_info_nce_logits(self, features):

        b_ = 0.5 * int(features.size(0))

        self_mask = torch.cat([torch.arange(b_) for i in range(self.args.client.n_views)], dim=0)
        self_mask = (self_mask.unsqueeze(0) == self_mask.unsqueeze(1)).float()
        self_mask = self_mask.to(self.device)
        ## mask on self and its different view
    ...
    def _algorithm(self, images, labels, uq_idxs, mask_lab, global_epoch=0) -> Dict:
        ...
        # represent clustering, unsup
        feature_clustering_logits, feature_clustering_labels = self.clustering_info_nce_logits(features=student_proj)
        feature_clustering_loss = torch.nn.CrossEntropyLoss()(feature_clustering_logits, feature_clustering_labels)
        ...
        losses["feat_cluster"] = feature_clustering_loss
