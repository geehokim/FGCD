...
class MetricExtraClient(MetricClient,ExtraInterpolateClient):
    ...
    def _algorithm(self, local_result, branch_result, labels,) -> Dict:

        losses = defaultdict(float)
        no_relu = not self.args.client.metric_loss.feature_relu

        cls_loss = self.criterion(local_result["logit"], labels)
        losses["cls"] = cls_loss

        losses.update(self._algorithm_metric(local_results = local_result, global_results = branch_result, labels=labels))               
        ...
        for local_epoch in range(self.args.trainer.local_epochs):
            ...
            for i, (images, labels) in enumerate(self.loader):
                ...
                with autocast(enabled=self.args.use_amp):
                    ...
                    losses = self._algorithm(results['local_result'], results['branch_result'], labels)            
