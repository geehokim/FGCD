...
class SimGCDOnlinePriorMatchingClients():
    ...
    def evaluate_local_trainset(self, epoch: int, global_epoch=0,
        ...
        umap_plot = self.evaler.visualize_local_umap(model=copy.deepcopy(self.model), client_idx=self.client_index, all_feats=results['all_feats'],
    ...
    def evaluate_local_trainset_unlabelled(self, epoch: int, global_epoch=0,
        ...
        wandb_dict = {
            ...
            f"{self.args.dataset.name}/client{self.client_index}/local_cluster_old_acc": results["cluster_old_acc"],
            f"{self.args.dataset.name}/client{self.client_index}/local_cluster_new_acc": results["cluster_new_acc"],
            f"{self.args.dataset.name}/client{self.client_index}/local_infomap_acc": results["infomap_acc"],
            f"{self.args.dataset.name}/client{self.client_index}/local_infomap_old_acc": results["infomap_old_acc"],
            f"{self.args.dataset.name}/client{self.client_index}/local_infomap_new_acc": results["infomap_new_acc"],
        }
        ...
        return {
            ...
            "local_infomap_acc": results["infomap_acc"],
            "local_infomap_old_acc": results["infomap_old_acc"],
            "local_infomap_new_acc": results["infomap_new_acc"],
        }
    ...
    def setup(self, model, device, local_dataset, global_epoch, local_lr, **kwargs):
        ...
        cluster_labels, act_protos, local_prototypes, labeled_prototypes = self.local_clustering(self.args, self.model, self.loader_for_clustering, self.evaler, global_epoch, self.device)
        ...
        self.model.proj_layer.act_protos.data = torch.tensor(act_protos).long()
        self.model.proj_layer.local_prototypes.data[:act_protos] = local_prototypes
    ...
    def get_weights(self, epoch=None):
        ...
        weights = {
            ...
            "global_cls": self.args.client.global_cls_weight,
        }
        
        return weights

    
    def local_clustering(self, args, model, loader, evaler, global_epoch, device, local_epoch=0):
        ...
        cluster_feature = feats_unlabelled.clone()
        ...
        local_prototypes = torch.Tensor(centroid).to(device)
        ...
        return sorted_cluster_labels, act_protos, local_prototypes, labeled_prototypes
    ...
    def local_train(self, global_epoch, **kwargs):
        ...
        for local_epoch in range(self.args.trainer.local_epochs):
            ...
            if self.args.trainer.local_eval and (local_epoch+ 1) % self.args.trainer.local_eval_freq == 0:
                self.evaluate_local_trainset_unlabelled(epoch=local_epoch, local_datasets=None)
    ...
    def _algorithm(self, images, labels, uq_idxs, mask_lab, global_epoch=0, local_epoch=0, online_prior=None) -> Dict:
        ...
        student_proj, student_out, student_out_clustered, student_out_global = self.model(images)
        teacher_out = student_out.detach()
        teacher_out_clustered = student_out_clustered.detach()
        teacher_out_clustered_unlabelled = torch.cat([f[mask_lab] for f in teacher_out_clustered.chunk(2)], dim=0)
        ...
        ### for labelled data
        teacher_out_labelled = torch.cat([f[mask_lab] for f in teacher_out.chunk(2)], dim=0)
        student_out_labelled = torch.cat([f[mask_lab] for f in student_out.chunk(2)], dim=0)
        cluster_loss_labelled = self.cluster_criterion(student_out_labelled, teacher_out_labelled, global_epoch)
        ...
        avg_probs = (student_out_clustered_unlabelled / 0.1).softmax(dim=1).mean(dim=0)
        ...
        # clustering, global, unsup
        if self.args.client.global_clustering and global_epoch > 0 and self.args.trainer.num_clients > 1:
            teacher_out_global = student_out_global.detach()
            cluster_loss_global = self.cluster_criterion(student_out_global, teacher_out_global, global_epoch)
            losses["global_cls"] = cluster_loss_global
