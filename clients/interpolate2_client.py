...
class Interpolater():
    ...
    def get_interpolate_model(self, factor, stochastic=False):
        ...
        for l, g, i in zip(self.local_model.modules(), self.global_model.modules(), self.inter_model.modules()):
            if isinstance(i, nn.Conv2d):
                # setattr(layer, f"weight1", alpha)
                setattr(i, f"weight1", g.weight)
                setattr(i, f"alpha", inter_factor)
...
class Interpolate2Client(Client):
    ...
    def local_train(self, global_epoch, **kwargs):
        ...
        for local_epoch in range(self.args.trainer.local_epochs):
            ...
            for i, (images, labels) in enumerate(self.loader):
                ...
                if client_args.get('label_noise'):
                    ...
                    # breakpoint()
                    labels = apply_label_noise(labels, 100, noise_ratio)
        ...
        if global_epoch > 0 and global_epoch % 20 == 0:
            ...
            #     loss_dict.update({
            #         f'acc/{self.args.dataset.name}/inter{factor}': inter_results["acc"],
            #         f'class_acc/{self.args.dataset.name}/inter{factor}/top{D}': inter_results["class_acc"][sorted_class_ids[:D]].mean(),
            #         f'class_acc/{self.args.dataset.name}/inter{factor}/else{D}': inter_results["class_acc"][sorted_class_ids[D:]].mean(),
            #     })
            ...
            #     loss_dict.update({
            #         f'acc/{self.args.dataset.name}/stoc_inter{factor}': stoc_inter_results["acc"],
            #         f'class_acc/{self.args.dataset.name}/stoc_inter{factor}/top{D}': stoc_inter_results["class_acc"][sorted_class_ids[:D]].mean(),
            #         f'class_acc/{self.args.dataset.name}/stoc_inter{factor}/else{D}': stoc_inter_results["class_acc"][sorted_class_ids[D:]].mean(),
            #     })
                
            
            # loss_dict.update({
            #     f'acc/{self.args.dataset.name}/local': local_results["acc"],
            #     f'class_acc/{self.args.dataset.name}/local/top{D}': local_results["class_acc"][sorted_class_ids[:D]].mean(),
            #     f'class_acc/{self.args.dataset.name}/local/else{D}': local_results["class_acc"][sorted_class_ids[D:]].mean(),
            # })
            # # print(loss_dict)
            # logger.warning(f'[C{self.client_index}, E{global_epoch}] Local Model: {local_results["acc"]:.2f}%, Inter Model: {inter_results["acc"]:.2f}%, Stoc Model: {stoc_inter_results["acc"]:.2f}%')
