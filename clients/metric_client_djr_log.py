...
class MetricClient_djr_log(Client):
    ...
    def setup(self, model, device, local_dataset, global_epoch, local_lr, trainer, **kwargs):
        ...
        #### For logging D_jr
        self.log_djr = False
        if self.args.get('Djr_freq'):
            if global_epoch % self.args.get('Djr_freq') == 0:
                self.log_djr = True
                self.djr_dict = {}
    ...
    def _algorithm_metric(self, local_results, global_results, labels, log_djr = False):
                            ...
                            if metric_criterion.sampling:
                                ...
                                if self.args.get('ml2'):
                                    ...
                                    loss_metric, self.feat_norm_stat, self.feat_norm_ratio_stat, self.feat_cos_stat = metric_criterion(old_feat=global_feature_l, new_feat=local_feature_l, target=labels, reduction=False, topk_neg=topk_neg,
                                                               uncertainty=uncertainty, class_ratio=class_ratio, level=l, progress=self.current_progress, djr =log_djr, class_major = self.batch_major_stat['this_batch_major'], class_minor = self.batch_minor_stat['this_batch_minor'])
    ...
    def _algorithm(self, images, labels,log_djr = False) -> Dict:
        ...
        losses.update(self._algorithm_metric(local_results=results, global_results=global_results, labels=labels,log_djr = log_djr))               
        #breakpoint()
        ...
        if self.log_djr:
            with torch.no_grad():
                for i, (images, labels) in enumerate(self.djrloader):
                    ...
                    self.model.zero_grad()
                    with autocast(enabled=self.args.use_amp):
                        losses, features = self._algorithm(images, labels, self.log_djr)
