...
class GCDClient_InfoMapCon():
    ...
    def __init__(self, args, client_index, loader=None, evaler=None):
        ...
        self.cluster_loss = ClusterLoss(temperature=self.args.client.cluster_temperature)
        return
    ...
    def setup(self, model, device, local_dataset, global_epoch, local_lr, trainer, **kwargs):
        ...
        self.cluster_loss.update_prototypes(cluster_prototypes, device=self.device)
    ...
    def semi_infomap(self, args, model, loader, evaler, num_classes, global_epoch, device):
        ...
        if args.client.cluster_feature == 'feats':
            cluster_feature = all_feats
        elif args.client.cluster_feature == 'feats_proj':
            cluster_feature = all_feats_proj
        else:
        ...
        # with temp_seed(1):
        feat_dists, feat_nbrs = get_dist_nbr(features=cluster_feature, k=args.client.k1, knn_method='faiss_gpu', device=device)
        ...
        wandb.log({
                f"{args.dataset.name}/global/infomap_all_acc": all_acc * 100,
                f"{args.dataset.name}/global/infomap_old_acc": old_acc * 100,
                f"{args.dataset.name}/global/infomap_new_acc": new_acc * 100,
            }, step=global_epoch)
        
        
        if args.client.cluster_feature == 'feats':
            return pseudo_labels, all_feats
        elif args.client.cluster_feature == 'feats_proj':
            return pseudo_labels, all_feats_proj
        else:
            raise ValueError(f"Invalid cluster feature: {args.client.cluster_feature}")
    ...
    def get_weights(self, epoch=None):
        ...
        weights = {
            ...
            "cluster_con": self.args.client.cluster_con_weight * self.contrastive_cluster_weight_schedule[epoch],
        }
