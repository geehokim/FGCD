...
class DistillLoss(nn.Module):
    def __init__(self, warmup_teacher_temp_epochs, nepochs,
                 ...
                 student_temp=0.1, ensemble_weight=0.5):
        super().__init__()
        self.student_temp = student_temp
        self.ens_w = ensemble_weight
        self.ncrops = ncrops
    ...
    def forward(self, student_output, teacher_output, global_output, epoch):
        """
        ...
        teacher_output = self.ens_w * teacher_output + (1 - self.ens_w) * global_output
...
class SimGCD_GT_Entopy_Ens_Client():
    ...
    def _algorithm(self, images, labels, uq_idxs, mask_lab, global_epoch=0) -> Dict:
        ...
        global_proj, global_out = self.global_model(images)
        ...
        # clustering, unsup
        cluster_loss = self.cluster_criterion(student_out, teacher_out, global_out, global_epoch)
        ...
        if self.args.client.distance_type == 'l2':
            ...
            avg_probs = torch.clamp(avg_probs, 1e-7, 1)
            prior_dist = torch.clamp(self.prior_dist, 1e-7, 1)
            avg_probs = avg_probs * 1 / (prior_dist) ** self.args.client.p
