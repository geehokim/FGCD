...
class SimGCDGTPClient():
    ...
    def _algorithm(self, images, labels, uq_idxs, mask_lab, global_epoch=0) -> Dict:
        ...
        ## regularization on the local prior distribution
        avg_probs = (student_out / 0.1).log_softmax(dim=1).mean(dim=0).unsqueeze(0)
        me_max_loss = self.kl_loss(avg_probs, self.prior_dist)
