...
class SimGCD_GM_ENS_Client():

    def __init__(self, args, client_index, loader=None, evaler=None):
        self.args = args
        self.client_index = client_index
        self.ensemble_weight = args.client.ensemble_weight
        # self.loader = loader  
        # self.model = model
    ...
    def _algorithm(self, images, labels, uq_idxs, mask_lab, global_epoch=0) -> Dict:
        ...
        _, global_teacher_out = self.global_model(images)
        global_teacher_out = global_teacher_out.detach()

        # logit ensemble with teacher_out and global_teacher_out
        teacher_out = self.ensemble_weight * global_teacher_out + (1 - self.ensemble_weight) * teacher_out
