...
class SimGCD_Update_GMPrior_Client():
    ...
    def evaluate_local_trainset(self, epoch: int, global_epoch=0, local_datasets: List[torch.utils.data.Dataset] = None) -> Dict:
        ...
        wandb_dict = {
            f"local_all_acc/{self.args.dataset.name}": all_acc,
            f"local_old_acc/{self.args.dataset.name}": old_acc,
            f"local_new_acc/{self.args.dataset.name}": new_acc,
        }
    ...
    def setup(self, model, device, local_dataset, global_epoch, local_lr, trainer, **kwargs):
        ...
        if global_epoch + 1 > self.args.client.update_warmup:
            self.update_prior()
    ...
    def local_train(self, global_epoch, **kwargs):
        ...
        for local_epoch in range(self.args.trainer.local_epochs):
            ...
            # self.scheduler.step()
            if self.args.client.update_prior and (local_epoch+ 1) % self.args.client.update_prior_freq == 0:
    ...
    def update_prior(self, epoch: int = 0):
        ...
        all_acc, old_acc, new_acc, _ = self.evaler.log_accs_from_preds(y_true=targets, y_pred=preds, mask=mask,
