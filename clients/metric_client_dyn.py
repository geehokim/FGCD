...
class MetricClient_dyn(Client):
    ...
    def setup(self, model, device, local_dataset, global_epoch, local_lr, trainer, past_local_deltas, user, **kwargs):
        ...
        #For FedDyn
        self.local_deltas = (past_local_deltas)
        self.user = user
        self.local_delta = copy.deepcopy(self.local_deltas[self.user])
        # print("self.local_deltasum")
        # x = 0
        # for n,p in self.local_delta.items():
        #     x += p.sum()
        #     #print(p.sum())
        # print(x)
