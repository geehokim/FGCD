...
class GCDClient_InfoMapCon_Proto():
    ...
    def setup(self, model, device, local_dataset, global_epoch, local_lr, trainer, **kwargs):
        ...
        self.cluster_loss = ClusterLossProto(num_features=cluster_prototypes.size(1), num_samples=cluster_prototypes.size(0), temperature=self.args.client.cluster_temperature, device=self.device)
        self.cluster_loss.prototypes.data = F.normalize(cluster_prototypes, dim=1)
