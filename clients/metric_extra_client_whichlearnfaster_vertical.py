...
class MetricExtraClient_whichlearnfaster_vertical(MetricClient,ExtraInterpolateClient):
    ...
    def setup(self, model, device, local_dataset, global_epoch, local_lr, trainer, **kwargs):
        ...
        #vertical
        self.feature_queue = {}
        for name in ['local_feature_queue','global_feature_queue','local_prototype_queue','feature_center','global_prototype_queue']:
            self.feature_queue[name] = deque()

        self.local_center_momentum = {}
    ...
    @torch.no_grad()
    def modify_feature_grad(self, grad):
        for key in self.feature_queue.keys():
            assert(len(self.feature_queue[key])>0)

        now_local_feat, now_global_feat, now_feat_center = self.feature_queue['local_feature_queue'].pop(), self.feature_queue['global_feature_queue'].pop(), self.feature_queue['feature_center'].pop()
        now_local_prototype = self.feature_queue['local_prototype_queue'].pop()
        now_global_prototype = self.feature_queue['global_prototype_queue'].pop()

        if self.args.client.vertical.use_local_proto:
            now_proto = now_local_prototype
        else:
            now_proto = now_global_prototype

        if self.args.client.vertical.use_local_feat:
            now_feat = now_local_feat
        else:
            now_feat = now_global_feat



        def decompose_parallel(selected_grad , diff, weight, eliminate = 'positive',debug = False, clip = -1):
            if debug:
                breakpoint()
            selected_grad = selected_grad.squeeze()
            normalized_diff = diff / (diff.norm(dim=1, keepdim=True) + 1e-10)
            dotproduct = torch.matmul(selected_grad, (normalized_diff).T)
            try:
                if dotproduct.shape[1]>1:
                    dotproduct = torch.diagonal(dotproduct, 0).unsqueeze(dim = 1)
            except:
                pass
            ...
            if clip > 0:
                # x = copy.deepcopy(dotproduct)
                # y = x.clamp(max = self.args.client.vertical.direction_clipping)
                dotproduct -= dotproduct.clamp(max = clip)
            # print("x.mean():",x.mean())
            # print('x.max():',x.max())
            # print("y.mean():",y.mean())
            # print('y.max():',y.max())
            # assert(y.max()<= self.args.client.vertical.direction_clipping)
            return dotproduct * weight * normalized_diff



        #norm_feat_center = now_feat_center / now_feat_center.norm()
        for c in range(self.num_classes):
            idxs = (self.labels == c)
            if sum(idxs) > 0:
                diff = now_proto[c]-now_feat_center
                if self.args.client.vertical.modify_protovar_grad:
                    selected_grad = grad[idxs]
                    grad[idxs] = grad[idxs] - (decompose_parallel(selected_grad, diff, self.args.client.vertical.modify_weight, clip = self.args.client.vertical.direction_clipping)).view(grad[idxs].shape)
  
                #breakpoint()
                if self.args.client.vertical.modify_samplevar_grad:
                    if self.args.client.vertical.local_center_batch:
                        if sum(idxs) == 1:
                            continue

                    selected_grad = grad[idxs]
                    selected_feat = now_feat[idxs]
                    selected_feat = selected_feat.view(len(selected_feat), -1)
                    #breakpoint()
                    # if self.args.client.vertical.reverse_samplevar:
                    #     sample_diff = selected_feat.squeeze() - now_proto[c] 
                    # else:
                    #     sample_diff = now_proto[c] - selected_feat.squeeze()
                    if self.args.client.vertical.reverse_samplevar:
                        sample_diff = selected_feat - now_proto[c] 
                    else:
                        sample_diff = now_proto[c] - selected_feat
                    if self.args.client.vertical.modify_protovar_grad:
                        sample_diff = sample_diff - decompose_parallel(sample_diff, diff, 1, eliminate='all')

                    grad[idxs] = grad[idxs] - (decompose_parallel(selected_grad, sample_diff, self.args.client.vertical.modify_weight, eliminate=self.args.client.vertical.samplevar_eli, clip = self.args.client.vertical.direction_clipping)).view(grad[idxs].shape)
                    #breakpoint()
                    #diff_samplevar =  
                    #print(c, grad[idxs].sum())
                    if grad[idxs].sum().isnan():
                        breakpoint()
    ...
    def _algorithm_metric(self, local_results, global_results, labels, global_feat_center = None):
        ...
        for l in range(self.num_layers):
            ...
            if train_layer:
                ...
                ############vertical
                if global_feat_center is not None:
                    if self.args.client.vertical.global_center_momentum>0 or self.args.client.vertical.global_model_client_all_data_mean:
                        feature_center = global_feat_center[f"layer{l}"]
                else:
                    #breakpoint()
                    if self.args.client.vertical.local_center_batch:
                        feature_center = loc_prototypes_mean
                    elif self.args.client.vertical.local_center_momentum > 0:
                        if f"layer{l}" not in self.local_center_momentum.keys():
                            self.local_center_momentum[f"layer{l}"] = loc_prototypes_mean
                        else:
                            self.local_center_momentum[f"layer{l}"] = self.args.client.vertical.local_center_momentum * self.local_center_momentum[f"layer{l}"] + (1-self.args.client.vertical.local_center_momentum) *loc_prototypes_mean
                        feature_center = self.local_center_momentum[f"layer{l}"]
                    elif self.args.client.vertical.global_model_client_barch_data_mean:
                        feature_center = glob_prototypes_mean
                    else:
                        raise ValueError

                self.feature_queue['local_feature_queue'].append(local_feature_l)
                self.feature_queue['global_feature_queue'].append(global_feature_l)
                self.feature_queue['local_prototype_queue'].append(loc_prototypes_)
                self.feature_queue['feature_center'].append(feature_center)
                self.feature_queue['global_prototype_queue'].append(glob_prototypes_)
                self.labels = labels
                ...
                #vertical
                local_feature_l.register_hook(self.modify_feature_grad)
