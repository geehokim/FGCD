...
class GCDSUIterativeClient():
    ...
    def setup(self, model, device, local_dataset, global_epoch, local_lr, trainer, **kwargs):
        ...
        param_groups_query = [
            {'params': self.model.base_vit.dynamic_unit_fusion.query_tokens, 'lr': local_lr * 250, 'weight_decay': 0},
            {'params': [p for n, p in self.model.base_vit.dynamic_unit_fusion.named_parameters() if "query_tokens" not in n], 'lr': local_lr,
             'weight_decay': self.args.optimizer.wd},

        ]
        self.optimizer_query = optim.SGD(param_groups_query, momentum=self.args.optimizer.momentum)
        param_groups_cls = [
        {'params': [p for n, p in self.model.named_parameters() if
                    "query_tokens" not in n and "dynamic_unit_fusion" not in n], 'lr': local_lr}]
        self.optimizer_cls = optim.SGD(param_groups_cls, lr=local_lr, momentum=self.args.optimizer.momentum,
                                   weight_decay=self.args.optimizer.wd)
    ...
    def local_train(self, global_epoch, **kwargs):
        ...
        for local_epoch in range(self.args.trainer.local_epochs):
            ...
            for i, (images, labels, uq_idxs, mask_lab) in tqdm(enumerate(self.loader), total=len(self.loader)):
                ...
                if (i + 1) % self.args.client.freq_rep_learn == 0:  # train classifier
                ...
                if switch_to_cls:  # train classifier
                    # freeze
                    self.model.base_vit.dynamic_unit_fusion.eval()
                    ...
                    try:
                        ...
                        scaler.unscale_(self.optimizer_cls)
                        if self.g_clipping:
                            torch.nn.utils.clip_grad_norm_(self.model.parameters(), 10)
                        scaler.step(self.optimizer_cls)
                        scaler.update()
                    ...
                    for name, m in self.model.base_vit.named_parameters():
                        if 'block' in name:
                            ...
                            if block_num >= 20:
                                m.requires_grad = True
                    self.model.base_vit.dynamic_unit_fusion.train()
                    for m in self.model.base_vit.dynamic_unit_fusion.parameters():
                        m.requires_grad = True
                    ...
                    try:
                        scaler.scale(loss).backward()
                        scaler.unscale_(self.optimizer_query)
                        if self.g_clipping:
                            torch.nn.utils.clip_grad_norm_(self.model.parameters(), 10)
                        scaler.step(self.optimizer_query)
                        scaler.update()
                    except Exception as e:
                        print(e)
    ...
    def info_nce_logits(self, features):
        ...
        logits = logits / self.args.client.temperature
    ...
    def _flush_memory(self):
        ...
        del self.optimizer_cls
        del self.optimizer_query
        del self.loader
