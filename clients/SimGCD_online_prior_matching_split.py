...
class SimGCDOnlinePriorMatchingSplitClients():
    ...
    def __init__(self, args, client_index, loader=None, evaler=None):
        ...
        self.num_seen_classes = len(self.args.dataset.seen_classes)
        return
    ...
    def setup(self, model, device, local_dataset, global_epoch, local_lr, **kwargs):
        ...
        cluster_labels, act_protos, local_prototypes = self.local_clustering_split(self.args, self.model, self.loader_for_clustering, self.evaler, global_epoch, self.device)
        self.cluster_labels = cluster_labels
        
        local_dataset_training = MergedDatasetCluster(copy.deepcopy(local_dataset.labelled_dataset), copy.deepcopy(local_dataset.unlabelled_dataset), cluster_labels=self.cluster_labels)
        ...
        self.model.proj_layer.local_prototypes.data[len(self.args.dataset.seen_classes):act_protos] = local_prototypes[:]
    ...
    def local_clustering_split(self, args, model, loader, evaler, global_epoch, device, local_epoch=0):
        ...
        feats_labelled = F.normalize(feats_labelled, dim=-1).cpu().numpy()
        feats_unlabelled = F.normalize(feats_unlabelled, dim=-1).cpu().numpy()

        # Calculate centroids for each labeled class
        unique_labels = np.unique(targets_labelled)
        labeled_centroids = []
        for label in unique_labels:
            mask = targets_labelled == label
            centroid = feats_labelled[mask].mean(axis=0, keepdims=True)
            centroid = F.normalize(torch.Tensor(centroid), dim=-1).cpu().numpy()
            labeled_centroids.append(centroid)
        labeled_centroids = np.concatenate(labeled_centroids, axis=0)  # [num_seen_classes, feature_dim]


        # Initialize sorted_cluster_labels with -1
        sorted_cluster_labels = np.full(len(feats_unlabelled), -1)

        # Calculate cosine similarities between unlabeled examples and labeled centroids
        similarities = np.dot(feats_unlabelled, labeled_centroids.T)  # [num_unlabeled, num_seen_classes]
        max_similarities = np.max(similarities, axis=1)  # [num_unlabeled]
        closest_centroids = np.argmax(similarities, axis=1)  # [num_unlabeled]  

        # Assign labels to high confidence examples
        high_conf_mask = max_similarities >= self.args.client.threshold
        sorted_cluster_labels[high_conf_mask] = unique_labels[closest_centroids[high_conf_mask]]
        
        ## 
        targets_unlabelled_high_conf = targets_unlabelled[high_conf_mask]
        # Calculate accuracy metrics for high confidence examples
        # 1. Check if the unlabeled examples are actually from seen classes
        actual_seen_mask = targets_unlabelled_high_conf < len(self.args.dataset.seen_classes)
        actual_seen_acc = actual_seen_mask.mean() * 100 if len(targets_unlabelled_high_conf) > 0 else 0
        
        # 2. Check if the assigned labels match the ground truth for high confidence examples
        correct_assignments = sorted_cluster_labels[high_conf_mask] == targets_unlabelled_high_conf
        assignment_acc = correct_assignments.mean() * 100 if len(targets_unlabelled_high_conf) > 0 else 0
        
        # Log the results
        logger.info(f"High confidence examples: {high_conf_mask.sum()} out of {len(high_conf_mask)}")
        logger.info(f"Actual seen class ratio: {actual_seen_acc:.2f}%")
        logger.info(f"Correct assignment accuracy: {assignment_acc:.2f}%")
        
        # Add to wandb if enabled
        if self.args.wandb:
            wandb.log({
                f"{self.args.dataset.name}/client{self.client_index}/high_conf_count": high_conf_mask.sum(),
                f"{self.args.dataset.name}/client{self.client_index}/actual_seen_acc": actual_seen_acc,
                f"{self.args.dataset.name}/client{self.client_index}/assignment_acc": assignment_acc
            }, step=global_epoch)



        # Get remaining examples for clustering
        remaining_mask = ~high_conf_mask
        remaining_features = feats_unlabelled[remaining_mask]

        if len(remaining_features) > 0:
            # Perform infomap clustering on remaining examples
            idx2label, idx_single, dists = cluster_by_infomap(remaining_features, 
                                                            k=args.client.top_k, 
                                                            tao_f=args.client.tao_f, 
                                                            dataset_name=args.dataset.name)
            
            # Remap cluster labels to be consecutive integers starting from num_seen_classes
            unique_cluster_labels = sorted(set(idx2label.values()))
            label_map = {old_label: new_label + len(self.args.dataset.seen_classes) 
                        for new_label, old_label in enumerate(unique_cluster_labels)}
            
            # Update idx2label with the new consecutive labels
            for idx in idx2label:
                idx2label[idx] = label_map[idx2label[idx]]
            
            # Create temporary array for remaining examples
            temp_cluster_labels = np.full(len(remaining_features), -1)
            for idx in idx2label:
                temp_cluster_labels[idx] = idx2label[idx]
                
            # Assign cluster labels to the original array maintaining order
            sorted_cluster_labels[remaining_mask] = temp_cluster_labels
            
            # Get centroids for new clusters
            new_centroids = []
            for label in range(len(self.args.dataset.seen_classes), max(label_map.values()) + 1):
                mask = temp_cluster_labels == label
                if np.any(mask):
                    centroid = remaining_features[mask].mean(axis=0, keepdims=True)
                    new_centroids.append(centroid)
            
            if new_centroids:
                new_centroids = np.concatenate(new_centroids, axis=0)
                all_centroids = np.concatenate([labeled_centroids, new_centroids], axis=0)
            else:
                all_centroids = labeled_centroids
        else:
            all_centroids = labeled_centroids

        
        estimate_k = new_centroids.shape[0]
        self.estimate_k = estimate_k
        print(f"Estimated number of clusters: {estimate_k}")
        act_protos = estimate_k + len(self.args.dataset.seen_classes)
        local_prototypes = torch.Tensor(new_centroids).to(device)

        # Calculate accuracies for this client
        unique_pseudo_labels, pseudo_label_counts = np.unique(sorted_cluster_labels, return_counts=True)
        ...
        all_acc, old_acc, new_acc, _, _ = evaler.log_accs_from_preds(
            y_true=targets_unlabelled,
            y_pred=sorted_cluster_labels,
            mask=client_mask_unlabelled,
            T=0,
            eval_funcs=['v2'],
        ...
        if args.wandb:
            wandb.log({
                    f"{args.dataset.name}/client{self.client_index}/infomap_unlabelled_all_acc": all_acc * 100,
                f"{args.dataset.name}/client{self.client_index}/infomap_unlabelled_old_acc": old_acc * 100,
                f"{args.dataset.name}/client{self.client_index}/infomap_unlabelled_new_acc": new_acc * 100,
            }, step=local_epoch)
        
        return sorted_cluster_labels, act_protos, local_prototypes
    ...
    def local_train(self, global_epoch, **kwargs):
        ...
        for local_epoch in range(self.args.trainer.local_epochs):
            ...
            for i, (images, labels, uq_idxs, mask_lab) in tqdm(enumerate(self.loader), total=len(self.loader)):
                ...
                online_prior = torch.zeros(self.estimate_k + self.num_seen_classes)
                epsilon = torch.ones(self.estimate_k + self.num_seen_classes) * 1e-7
