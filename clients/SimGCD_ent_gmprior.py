...
class SimGCD_GM_Entopy_Client():
    ...
    def setup(self, model, device, local_dataset, global_epoch, local_lr, trainer, **kwargs):
        ...
        ## Calculate Local Prior using Current Global model ##
        # Init global model
        self.global_model = copy.deepcopy(model)
        for fixed_model in [self.global_model]:
            for n, p in fixed_model.named_parameters():
        ...
        # Init global model
        # Extract logits of local data using the global model
        all_logits = self._extract_local_logits()
        ...
        if self.args.client.softmax_prior:
            ...
            for pred in preds:
                ...
                smooth_values[int(pred)] = smooth_max
                prior += smooth_values
            prior = prior.float() / len(preds)
    ...
    def get_weights(self, epoch=None):
        ...
        weights = {
            ...
            "memax": self.args.client.memax_weight * (1 - self.args.client.sup_con_weight),
        }
    ...
    @torch.no_grad()
    def _extract_local_logits(self):
        pass
        self.global_model.eval()
        self.global_model.to(self.device)
        test_transform = copy.deepcopy(self.evaler.test_loader.dataset.transform)
        tmp_loader = copy.deepcopy(self.loader)
        tmp_loader.dataset.labelled_dataset.transform = test_transform
        tmp_loader.dataset.unlabelled_dataset.transform = test_transform

        ## transform을 Test trainsform을 써야해

        all_logits = []
        all_labels = []
        for i, (images, labels, uq_idxs, mask_lab) in tqdm(enumerate(tmp_loader), total=len(tmp_loader)):

            images, labels = images.to(self.device), labels.to(self.device)
            mask_lab = mask_lab[:, 0]
            mask_lab = mask_lab.to(self.device).bool()

            _, logits = self.global_model(images)
