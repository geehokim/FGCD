...
__all__ = ['MultiLabelCrossEntropyLoss','MetricLoss', 'MetricLoss2', 'MetricLoss_rel', 'TripletLoss', 'IL','CE','IL_negsum', 'DeepInversionFeatureHook', 'LossManager']
...
class MultiLabelCrossEntropyLoss(nn.Module):
    ...
    uncertainty: torch.Tensor = None, class_ratio: torch.Tensor = None, level: float = None, progress: float = None,
                data_label: torch.Tensor = None, p_margin: torch.Tensor = None, n_margin: torch.Tensor = None, ) -> torch.Tensor:
        """Apply forward pass.
        ...
        if self.adapt_ce is not None:
            if self.adapt_ce == 'temp': # sqrt_norm
            ...
            elif self.adapt_ce == 'negative_margin':
                sim = input * self.temp
                negative_sim = sim[targets == 0]
                clamped_sim = torch.clamp(negative_sim - self.n_margin, min=-1)
                sim[targets == 0] = clamped_sim
                input = sim / self.temp

            elif self.adapt_ce == 'both_margin':
                sim = input * self.temp
                positive_sim = sim[targets == 1]
                clamped_p_sim = torch.clamp(positive_sim + self.p_margin, max=1)
                sim[targets == 1] = clamped_p_sim


                negative_sim = sim[targets == 0]
                clamped_n_sim = torch.clamp(negative_sim - self.n_margin, min=-1)
                sim[targets == 0] = clamped_n_sim
                input = sim / self.temp

            elif self.adapt_ce == 'learnable_positive_margin':
                assert p_margin is not None
                sim = input * self.temp
                positive_sim = sim[targets == 1]
                clamped_p_sim = torch.clamp(positive_sim + p_margin, max=1)
                sim[targets == 1] = clamped_p_sim
                input = sim / self.temp

            elif self.adapt_ce == 'learnable_both_margin':
                assert p_margin is not None
                assert n_margin is not None
                sim = input * self.temp
                positive_sim = sim[targets == 1]
                clamped_p_sim = torch.clamp(positive_sim + p_margin, max=1)
                sim[targets == 1] = clamped_p_sim
                ...
                negative_sim = sim[targets == 0]
                clamped_n_sim = torch.clamp(negative_sim - n_margin, min=-1)
                sim[targets == 0] = clamped_n_sim
                input = sim / self.temp
...
class MetricLoss(nn.Module):
    ...
    uncertainty=None, class_ratio=None, level=None, progress=None, name="loss1", p_margin=None, n_margin=None):
        if old_feat.dim() > 2:
        ...
        reduction=reduction, beta=self.beta, uncertainty=unc, class_ratio=class_ratio, level=level, progress=progress, data_label= target,
                              p_margin=p_margin, n_margin=n_margin)
...
class MetricLoss2(MetricLoss):
    ...
    def forward(self, old_feat, new_feat, target, reduction=True, pair=None, topk_pos=None, topk_neg=None, 
    uncertainty=None, class_ratio=None, level=None, progress=None, name="loss1", mode = "", p_margin=None, n_margin=None):
        if old_feat.dim() > 2:
            old_feat = old_feat.squeeze(-1).squeeze(-1)
        if new_feat.dim() > 2:
            new_feat = new_feat.squeeze(-1).squeeze(-1)
