...
class ResNet(nn.Module):
    ...
    def update_interpolation_model(self,low = 0, high = 0, interpolation_type = None, interpolation_model='current',
        ...
        in_weight = torch.FloatTensor(1).uniform_(0, 1).item()
                    ...
                    if 'fisher' in interpolation_type:
                        ...
                        if interpolation_model == 'current':
                            ...
                            this_dict[key] = this_dict[this_global_name] + weight * (
                                    this_dict[this_prev_name] - this_dict[this_global_name])
                        elif interpolation_model == 'local_diff':
                            # print("interpolation_model : ", interpolation_model)
                            this_dict[key] = this_dict[this_prev_name] + \
                                weight * ( this_dict[this_layer_name] - this_dict[this_prev_name] )
                        elif interpolation_model == 'global+local_diff':
                            # print("interpolation_model : ", interpolation_model)
                            this_dict[key] = this_dict[this_global_name] + \
                                weight * ( this_dict[this_layer_name] - this_dict[this_prev_name] )
                        elif interpolation_model == 'global+current+local_diff':

                            this_dict[key] = this_dict[this_global_name] + \
                                weight * ( in_weight * (this_dict[this_layer_name] - this_dict[this_prev_name]) +\
                                          (1-in_weight) * (this_dict[this_layer_name] - this_dict[this_global_name])
                                          )
                            # print(weight, in_weight)
                            # breakpoint()
    ...
    def mlb_forward(self, x, return_feature=False, selected_layer='', selected_layers=[], stochastic="deterministic", num_of_stochastic_branch=1, **kwargs):
        if stochastic == "deterministic":    
            ...
            if return_feature:
                ...
                layer_dict = {layer_name: ['l', 'i'] for layer_name in layers_list}
            ...
            if return_feature:
                ...
                layers_list = ['layer1', 'layer2', 'layer3', 'layer4', 'fc']
                layer_dict = {layer_name: ['l', 'g'] for layer_name in layers_list}
...
def ResNet18_GFLN_prev(num_classes=10, l2_norm=False, use_pretrained = False, transfer_learning = True, use_bn = False, use_pre_fc = False, use_bn_layer = False):
    #
