...
class ResNet18_GFLN(ResNet):
    def __init__(self, args, num_classes=10, **kwargs):
        super().__init__(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, l2_norm=args.l2_norm, use_pretrained=args.use_pretrained,
                   use_bn_layer=args.use_bn_layer)
        self.inter = False
        self.fi = None
    ...
    def mlb_forward(self, x, return_feature=False, selected_layer='', selected_layers=[], stochastic="deterministic",
                    num_of_stochastic_branch=1, **kwargs):
        if stochastic == "deterministic":
            ...
            if return_feature:
                ...
                layer_dict = {layer_name: ['l', 'g'] for layer_name in
                              layers_list}  # ['layer1', 'layer2', 'layer3', 'layer4', 'fc']}
    ...
    def metric_forward_feature_reluX(self, x, return_feature=False, level=0, return_feature_norelu=False):
        ...
        # breakpoint()
        if level <= 1:
            out1 = self.layer_forward_relu(self.layer1, F.relu(out0) if return_feature_norelu else out0,
                                           return_feature_norelu=return_feature_norelu)
        else:
            out1 = out0
        if level <= 2:
            out2 = self.layer_forward_relu(self.layer2, F.relu(out1) if return_feature_norelu else out1,
                                           return_feature_norelu=return_feature_norelu)
        else:
            out2 = out1
        if level <= 3:
            out3 = self.layer_forward_relu(self.layer3, F.relu(out2) if return_feature_norelu else out2,
                                           return_feature_norelu=return_feature_norelu)
        else:
            out3 = out2

        if level <= 4:
            out4 = self.layer_forward_relu(self.layer4, F.relu(out3) if return_feature_norelu else out3,
                                           return_feature_norelu=return_feature_norelu)
            # breakpoint()
            if return_feature_norelu:
    ...
    def forward(self, x, return_feature=False, level=0, stochastic="deterministic", num_of_stochastic_branch=1,
                do_mlb=False, return_feature_norelu=False, selected_layer="", selected_layers=[]):
        # breakpoint()
        # (self, x, return_feature=False, selected_layer='', stochastic="deterministic", num_of_stochastic_branch=1, **kwargs)
        if do_mlb:
            result = self.mlb_forward(x=x, return_feature=return_feature, stochastic=stochastic,
                                      num_of_stochastic_branch=num_of_stochastic_branch, selected_layer=selected_layer,
                                      selected_layers=selected_layers)
        else:
            if return_feature_norelu == True:
                result = self.metric_forward_feature_reluX(x, return_feature, level=level,
                                                           return_feature_norelu=return_feature_norelu)
            else:
