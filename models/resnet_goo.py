...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, l2_norm=False):
        ...
        if l2_norm:
            ...
            self.linear1 = nn.Linear(64 * block.expansion, num_classes, bias=False)
            self.linear2 = nn.Linear(128 * block.expansion, num_classes, bias=False)
            self.linear3 = nn.Linear(256 * block.expansion, num_classes, bias=False)
        else:
    ...
    def forward(self, x, return_feature=False,return_intermediate_logit = False):
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out1 = F.avg_pool2d(out, 32)
        out1 = out1.view(out1.size(0), -1)
        logit1 = self.linear1(out1)
        
        out = self.layer2(out)
        out2 = F.avg_pool2d(out, 16)
        out2 = out2.view(out2.size(0), -1)
        logit2 = self.linear2(out2)
        
        out = self.layer3(out)
        out3 = F.avg_pool2d(out, 8)
        out3 = out3.view(out3.size(0), -1)
        logit3 = self.linear3(out3)
        ...
        if return_feature==True:
            ...
            return logit'''
        if return_intermediate_logit == True:
            return logit1,logit2,logit3,logit
        else:
            return logit
...
def ResNet18_goo(num_classes=10, l2_norm=False):
