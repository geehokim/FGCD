...
class ResNet(nn.Module):
    ...
    def cal_mean_cossim_feature(self,x,y,size):
        
        x_flat = x.view(size,-1)
        y_flat = y.view(size,-1)
        return self.cos(x_flat,y_flat).mean()
    
    
    def cal_layer_mean_feature_cossim_loss(self,concat_feature,size):
        this_loss = 0
        base_f = concat_feature[-size:]
        num_of_feature = int(len(concat_feature)/size)
        for i in range(num_of_feature-1):
            this_f = concat_feature[i*size : (i+1)*size]
            this_loss += self.cal_mean_cossim_feature(this_f,base_f,size)
        this_loss /= num_of_feature
        return this_loss

    def get_cos_all_loss(self):
        return self.cos_all_loss
    def forward(self, x, return_feature=False,level = 0):
        if return_feature:
            ...
            this = torch.cat((out1_g,out1),dim=0)
            cos_all_loss.append(self.cal_layer_mean_feature_cossim_loss(this,size))
            
            
            out2_g = self.global_layer2(this)
            out2 = self.layer2(this)
            
                               
                               
                               
                               
            this = torch.cat((out2_g,out2),dim=0)
            cos_all_loss.append(self.cal_layer_mean_feature_cossim_loss(this,size))
                                
            out3_g = self.global_layer3(this)
            out3 = self.layer3(this)

            this = torch.cat((out3_g,out3),dim=0)
            cos_all_loss.append(self.cal_layer_mean_feature_cossim_loss(this,size))
                               
                               
            out4_g = self.global_layer4(this)
            out4_g = F.adaptive_avg_pool2d(out4_g, 1)
            out4_g = out4_g.view(out4_g.size(0), -1)
            out4 = self.layer4(this)
            out4 = F.adaptive_avg_pool2d(out4, 1)
            out4 = out4.view(out4.size(0), -1)

            #print("out4_g : ",out4_g.shape)
            #print("out4 : ",out4.shape)
            this = torch.cat((out4_g,out4),dim=0)
            cos_all_loss.append(self.cal_layer_mean_feature_cossim_loss(this,size))
            logit_g = self.global_fc(this)
            logit = self.fc(this)
            
            self.cos_all_loss = sum(cos_all_loss)/len(cos_all_loss)
...
def ResNet18_GFLN_parallel_allcase_exceptfirst_featurecossim(num_classes=10, l2_norm=False, use_pretrained = False, transfer_learning = True, use_bn = False, use_pre_fc = False, use_bn_layer = False):
