...
class ResNet(nn.Module):
    ...
    def mlb_forward(self, x, return_feature=False, level = 0, stochastic = "deterministic", num_of_stochastic_branch = 1):
    ...
    def forward(self, x, return_feature=False, level = 0, stochastic = "deterministic", num_of_stochastic_branch = 1, do_mlb = False):
        
        if do_mlb:
            self.mlb_forward(x, return_feature, level, stochastic , num_of_stochastic_branch)
        else:
            self.metric_forward(x, return_feature)
...
def ResNet18_GFLN_parallel_allcase_exceptfirst_inter(num_classes=10, l2_norm=False, use_pretrained = False, transfer_learning = True, use_bn = False, use_pre_fc = False, use_bn_layer = False):
    #
