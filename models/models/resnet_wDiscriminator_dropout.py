...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, l2_norm=False, use_pretrained = False, use_bn_layer = False):
        ...
        self.discriminator =  nn.Sequential(
                                            nn.Linear(num_classes, mlp_hidden_size),
                                            #nn.GroupNorm(2, mlp_hidden_size),
                                            
                                            nn.ReLU(inplace=True),
                                            nn.Dropout(0.5),
                                            nn.Linear(mlp_hidden_size, projection_size),
                                            #nn.GroupNorm(2, projection_size),
                                            nn.ReLU(inplace=True),
                                            nn.Dropout(0.5),
    ...
    def freezeDis_unfreezeElse(self):
        ...
        for name,par in self.named_children():
            if 'discriminator' in name:
                par.eval()
            else:
                par.train()

    def unfreezeDis_freezeElse(self):
        for name,par in self.named_parameters():
            if 'discriminator' in name:
                par.requires_grad = True
            else:
...
def ResNet18_wDiscriminator_dropout(num_classes=10, l2_norm=False, use_pretrained = False, transfer_learning = True, use_bn = False, use_pre_fc = False, use_bn_layer = False):
