...
class ResNet(nn.Module):
    ...
    def forward_mixup(self, x, return_feature=False, level = 0, beta=0.5):
        if return_feature:

            out0 = F.relu(self.bn1(self.conv1(x)))
            out0_g = F.relu(self.global_bn1(self.global_conv1(x)))
            
            
            out0_mix = out0*beta + out0_g*(1-beta)

            out1_g = self.global_layer1(out0_mix)
            
            
            out1 = self.layer1(torch.cat((out0_g,out0),dim=0))
            #print("before concat",out1.shape)
            #print("after concat",torch.cat((out1_g,out1),dim=0).shape)
            out2_g = self.global_layer2(torch.cat((out1_g,out1),dim=0))
            out2 = self.layer2(torch.cat((out1_g,out1),dim=0))

            out3_g = self.global_layer3(torch.cat((out2_g,out2),dim=0))
            out3 = self.layer3(torch.cat((out2_g,out2),dim=0))
            ...
            out4 = self.layer4(torch.cat((out3_g,out3),dim=0))
            ...
            logit = self.fc(torch.cat((out4_g,out4),dim=0))
    ...
    def forward(self, x, return_feature=False,level = 0):
        ...
        if return_feature:
            ...
            out1_g = self.global_layer1(torch.cat((out0_g,out0),dim=0))
            out1 = self.layer1(torch.cat((out0_g,out0),dim=0))
            #print("before concat",out1.shape)
            #print("after concat",torch.cat((out1_g,out1),dim=0).shape)
            out2_g = self.global_layer2(torch.cat((out1_g,out1),dim=0))
...
def ResNet18_GFLN_parallel_allcase(num_classes=10, l2_norm=False, use_pretrained = False, transfer_learning = True, use_bn = False, use_pre_fc = False, use_bn_layer = False):
