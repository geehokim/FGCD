...
class ResNet(nn.Module):
    ...
    def get_rand(self, x):
        randsize = list(torch.ones(len(x.shape),dtype = int))
        randsize[0] = len(x)
        rand_sample = torch.rand(randsize).to(x.device)
        return rand_sample
    
    
    def do_mix_feature(self, out,out_g):
        rand_sample = self.get_rand(out)
        out_mixed = self.get_mixed_feature(out,out_g,rand_sample)
        output = torch.cat((out_mixed,out_g,out), dim = 0)
        return output
    ...
    def forward(self, x, return_feature=False,level = 0):
        if return_feature:
            ...
            out1_all = self.do_mix_feature(out1,out1_g)
            
            
            out2_g = self.global_layer2(out1_all)
            out2 = self.layer2(out1_all)
            out2_all = self.do_mix_feature(out2,out2_g)
                               
                               

            out3_g = self.global_layer3(out2_all)
            out3 = self.layer3(out2_all)
            out3_all = self.do_mix_feature(out3,out3_g)


            out4_g = self.global_layer4(out3_all)
            out4_g = F.adaptive_avg_pool2d(out4_g, 1)
            out4_g = out4_g.view(out4_g.size(0), -1)
            out4 = self.layer4(out3_all)
            out4 = F.adaptive_avg_pool2d(out4, 1)
            out4 = out4.view(out4.size(0), -1)
            out4_all = self.do_mix_feature(out4,out4_g)

            #print("out4_g : ",out4_g.shape)
            #print("out4 : ",out4.shape)

            logit_g = self.global_fc(out4_all)
            logit = self.fc(out4_all)
...
def ResNet18_GFLN_parallel_allcase_exceptfirst_mixedfeature(num_classes=10, l2_norm=False, use_pretrained = False, transfer_learning = True, use_bn = False, use_pre_fc = False, use_bn_layer = False):
