...
class BaseTrainer():
    ...
    def __init__(self,
        ...
        dataset_unlabelled_eval = self.local_dataset_split_ids['total_train_dataset_unlabelled']
        dataset_unlabelled_eval.transform = self.datasets['test'].transform

        test_loader = DataLoader(dataset_unlabelled_eval,
        ...
        if self.args.dataset.test_semisup:
            test_labelled_loader = DataLoader(self.datasets["test_labelled"],
                                 batch_size=args.evaler.batch_size if args.evaler.batch_size > 0 else args.batch_size,
                                 shuffle=False, num_workers=args.num_workers)
            eval_params["test_labelled_loader"] = test_labelled_loader
            test_unlabelled_loader = DataLoader(self.datasets["test_unlabelled"],
                                 batch_size=args.evaler.batch_size if args.evaler.batch_size > 0 else args.batch_size,
                                 shuffle=False, num_workers=args.num_workers)
            eval_params["test_unlabelled_loader"] = test_unlabelled_loader
    ...
    def local_update(self, device, task_queue):
        ...
        return local_model_state_dict, local_loss_dict, optimizer_state_dict
    ...
    def train(self) -> Dict:
        ...
        if self.args.eval_first:
            self.evaluate(epoch=0, local_models=[], local_datasets=None)
        ...
        for epoch in range(self.start_round, self.global_rounds):
            ...
            for i, client_idx in enumerate(selected_client_ids):
                ...
                local_state_dict, local_loss_dict, local_optimizer_state_dict = self.local_update(self.device, task_queue_input)
            ...
            # Server-side
            updated_global_state_dict, _, updated_optimizer_state_dict = self.server.aggregate(local_weights, local_deltas, local_optimizer_state_dicts, selected_client_ids, 
                                                                 copy.deepcopy(global_state_dict), current_lr=current_lr, epoch=epoch)
    ...
    def save_model(self, local_models, prev_global_model_state=None, epoch: int = -1, suffix: str = '') -> None:
        ...
        save_checkpoint(self.model, local_models, prev_global_model_state, model_path, epoch, save_torch=True, use_breakpoint=False)
        return
    ...
    def evaluate(self, epoch: int, local_models, local_datasets: List[torch.utils.data.Dataset] = None, eval_type='logits') -> Dict:

        if self.args.evaler.type == "FinchEvaler":
            if self.args.dataset.test_semisup:
                results = self.evaler.eval_semisup(model=self.model, epoch=epoch)
            else:
            ...
            elif eval_type == 'finch':
                results = self.evaler.eval_cluster(model=self.model, epoch=epoch)
            all_acc = results["all_acc"]
            new_acc = results["new_acc"]
            old_acc = results["old_acc"]
