...
class LocalTrainer():
    ...
    def local_update(self, device, task_queue):
        ...
        local_model_state_dict, local_loss_dict = client.local_train(global_epoch=task['global_epoch'])
        return local_model_state_dict, local_loss_dict
    ...
    def train(self) -> Dict:
        ...
        self.save_model(epoch=0, local_models=[self.model.state_dict(), self.model.state_dict()])
        ...
        for epoch in range(self.start_round, self.global_rounds):
            ...
            for i, client_idx in enumerate(selected_client_ids):
                ...
                local_state_dict, local_loss_dict = self.local_update(self.device, task_queue_input)
            ...
            self.save_model(epoch=epoch, local_models=local_models)
    ...
    def save_model(self, epoch: int = -1, suffix: str = '', local_models: List = None) -> None:
        ...
        local_dict = {i: local_models[i] for i in range(len(local_models))}
        if not model_path.parent.exists():
            model_path.parent.mkdir(parents=True, exist_ok=True)
        torch.save(local_dict, model_path)
        #save_checkpoint(local_dict, model_path, epoch, save_torch=True, use_breakpoint=False)
        logger.info(f'Local models are save to {model_path}')
        return
