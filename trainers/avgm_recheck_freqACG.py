...
from utils import terminate_processes, initalize_random_seed, save_checkpoint, save_checkpoint_momentum
...
class avgm_recheck_freqACG():
    ...
    def __init__(self,
        ...
        self.serverAvgM = serverAvgM
        self.serverACG = serverACG
        self.serverlist = [self.serverACG, self.serverAvgM]
        self.servernamelist = ['FedACG','FedAvgM']
        for server in self.serverlist:
            if server.args.server.momentum > 0:
                server.set_momentum(self.model)
        
        if self.args.get("FedACG_freq"):
            self.ACGfreq=self.args.FedACG_freq
    ...
    def train(self) -> Dict:
        ...
        for epoch in range(self.start_round, self.global_rounds):
            ...
            # if self.args.get('sameFedAVgMmomentum'):
            #     self.serverACG.global_delta = copy.deepcopy(self.serverAvgM.global_delta)
            #     self.serverACG.global_momentum = copy.deepcopy(self.serverAvgM.global_momentum)
            ...
            if epoch % self.ACGfreq == 0:
                this_serverlist = [self.serverACG]
                this_servernamelist = ['FedACG'] 
                if self.ACGfreq > 1:
                    self.serverACG.global_delta = copy.deepcopy(self.serverAvgM.global_delta)
                    self.serverACG.global_momentum = copy.deepcopy(self.serverAvgM.global_momentum)

            else:               
                this_serverlist = [self.serverAvgM]
                this_servernamelist = ['FedAvgM']
                if (epoch - 1) % self.ACGfreq == 0:
                    self.serverAvgM.global_delta = copy.deepcopy(self.serverACG.global_delta)
                    self.serverAvgM.global_momentum = copy.deepcopy(self.serverACG.global_momentum)                



            for server, servername in zip(this_serverlist, this_servernamelist):
                print(servername)
                ...
                print("Do FedACG?", server.args.server.get('FedACG')) 
                if server.args.server.get('FedACG'):
                    assert(self.args.server.momentum > 0)
                    self.model= copy.deepcopy(server.FedACG_lookahead(copy.deepcopy(self.model)))
                    global_state_dict = copy.deepcopy(self.model.state_dict())
                ...
                # Server-side
                updated_global_state_dict = server.aggregate(local_weights, local_deltas,
                ...
                if self.args.eval.freq > 0 and epoch % self.args.eval.freq == 0:
                    wandb_dict.update(self.evaluate(epoch=epoch, local_datasets=local_datasets))
                ...
                if (self.args.save_freq > 0 and (epoch + 1) % self.args.save_freq == 0) or (epoch + 1 == self.args.trainer.global_rounds):
                    self.save_model(delta =server.global_delta ,momentum =server.global_momentum ,epoch=epoch)

                
                delta_norm, momentum_norm = server.get_norm_stat()  
                wandb_dict['delta_norm'] = delta_norm
                wandb_dict['momentum_norm'] = momentum_norm

                modify_wandb_dict = {}
                for key in wandb_dict.keys():
                    modify_wandb_dict[servername + key] = wandb_dict[key]
    ...
    def save_model(self, delta, momentum, epoch: int = -1, suffix: str = '') -> None:
        ...
        save_checkpoint_momentum(self.model, model_path, epoch, delta= delta, momentum = momentum, save_torch=True, use_breakpoint=False)        
        return
