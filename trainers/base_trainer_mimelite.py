...
class Trainer_mimelite():
    ...
    def __init__(self,
        ...
        self.global_optimizer_state = copy.deepcopy(local_g)
    ...
    def local_update(self, device, task_queue, result_queue):
        ...
        while True:
            ...
            if self.args.client.get('mimelite'):
                setup_inputs['global_optimizer_state'] = self.global_optimizer_state
                #setup_inputs['user'] = task['user']
            ...
            # Local Training
            if self.args.client.get('mimelite'):
                local_model, local_loss_dict, full_grad_model_dict = client.local_train(global_epoch=task['global_epoch'])
                result_queue.put((local_model, local_loss_dict, full_grad_model_dict))
            else:
    ...
    def train(self) -> Dict:
        ...
        for epoch in range(self.start_round, self.global_rounds):
            ...
            local_optimizer_states = defaultdict(list)
            ...
            for i, client_idx in enumerate(selected_client_ids):
                ...
                if self.args.client.get('mimelite'):
                    task_queue_input['user'] = client_idx
                if self.args.multiprocessing:
                    ...
                    if self.args.client.get('mimelite'):
                        local_state_dict, local_loss_dict, full_grad_model_dict = result_queue.get()
                    else:
                    ...
                    for param_key in local_state_dict:
                        ...
                        if self.args.client.get('mimelite'):
                            local_optimizer_states[param_key].append(-(full_grad_model_dict[param_key]-global_state_dict[param_key]))
            ...
            if self.args.multiprocessing:
                for _ in range(len(selected_client_ids)):
                    ...
                    if self.args.client.get('mimelite'):
                        local_state_dict, local_loss_dict, full_grad_model_dict = result
                    else:
            ...
            #Serverside mimelite update
            global_optimizer_state_sum = 0
            if self.args.client.get('mimelite'):
                for key in self.global_optimizer_state.keys():
                    self.global_optimizer_state[key] = self.args.client.mimelite.beta * self.global_optimizer_state[key] + (1- self.args.client.mimelite.beta) * sum(local_optimizer_states[key])/len(local_optimizer_states[key])
                    global_optimizer_state_sum += torch.sum(self.global_optimizer_state[key]**2)
            print("global_optimizer_state_sum : ", global_optimizer_state_sum)
            # Server-side
