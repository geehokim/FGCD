...
class Trainer_whichlearnfaster():
    ...
    def train(self) -> Dict:
        ...
        for epoch in range(self.start_round, self.global_rounds):
            ...
            saved_client_ids[epoch] = selected_client_ids
        ...
        self.model = copy.deepcopy(initial_global_model)
        models_dict = {}
        models_dict['saved_last_model'] = last_global_model
        ...
        for epoch in range(self.start_round, self.global_rounds):
            ...
            #     else:
            #         selected_client_ids = range(len(self.clients))
            selected_client_ids = saved_client_ids[epoch] 
            logger.info(f"Global epoch {epoch}, Selected client : {selected_client_ids}")
            ...
            if self.args.analysis:
                ...
                if True:
                    models_dict['model_now'] = copy.deepcopy(self.model)
                    cal_distances_between_models(models_dict)
                    for name in ['distance_current_last','ratio_distance_current_last']:
                        for key in models_dict[name].keys():
                            this_str = str(name) + "/" + str(key)
                            wandb_dict[this_str] = models_dict[name][key]
