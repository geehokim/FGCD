...
from utils.helper import cal_distances_between_models
...
class Trainer_whichlearnfaster():
    ...
    def train(self) -> Dict:
        ...
        initial_global_model = copy.deepcopy(self.model)


        models_dict = {}
        models_dict['saved_initial_model'] = initial_global_model
        #Second, compare with final model
        for epoch in range(self.start_round, self.global_rounds):
            ...
            if self.args.analysis:
                ...
                if self.args.svd.freq > 0 and epoch % self.args.svd.freq == 0:
                ...
                if epoch > 0 and self.args.umap.freq > 0 and epoch % self.args.umap.freq == 0:
                    self.visualize_umap(global_model=self.model, local_models=local_models, epoch=epoch)
            ...
            if self.args.visualize_prototype.freq>0 and epoch%self.args.visualize_prototype.freq ==0:
                self.visualize_prototype(epoch=epoch, local_models=local_models, global_model=self.model, local_datasets=local_datasets)




            if epoch % self.args.save_for_distance_freq == 0:
                this_str = 'saved_model_at_' + str(epoch)
                models_dict[this_str] = {}
                models_dict[this_str]['epoch'] = epoch
        ...
        # last calculate distance between models.
            ...
            if 'saved_model_at_' in k:
                ...
                for name in ['distance_current_last','ratio_distance_current_last']:
                    for key in models_dict[name].keys():
                        this_str = str(name) + "/" + str(key)
                        distance_dict[this_str] = models_dict[name][key]

                print(k)
                print(models_dict[k]['epoch'])
                self.wandb_log(distance_dict, step = models_dict[k]['epoch'] + epoch)
    ...
    def evaluate_local(self, epoch: int, local_models: List[nn.Module], ) -> Dict:
        results = self.evaler.eval(model=local_models[0], epoch=epoch)
        acc = results["acc"]
        logger.warning(f'  [Epoch {epoch}] Local Test Accuracy: {acc:.2f}%')

        self.wandb_log({f"acc/{self.args.dataset.name}/local": acc}, step=epoch)
        return {
            "acc": acc
        }
    ...
    def visualize_landscape(self, global_model: nn.Module, local_models: List[nn.Module], prev_model_weight: Any,  epoch: int, ):
        ...
        model_files = copy.deepcopy([local_model.state_dict() for local_model in local_models])
    ...
    def visualize_umap(self, global_model: nn.Module, local_models: List[nn.Module], epoch: int, ):
        umap_results = self.evaler.visualize_umap(global_model=global_model, local_models=local_models, epoch=epoch)
    ...
    def visualize_prototype(self, epoch: int, local_models: List[nn.Module], global_model: nn.Module, local_datasets: List[torch.utils.data.Dataset], device: torch.device = None):
        results = self.evaler.visualize_prototype(epoch, local_models, global_model, local_datasets)
        self.wandb_log(results, step = epoch)
