...
class onlyumap():
    ...
    def __init__(self,
        ...
        if self.args.get('load_model_path2'):
            self.model2 = copy.deepcopy(self.model)
            self.load_model( mode=2)
    ...
    def train(self) -> Dict:
        ...
        self.visualize_umap(global_model=self.model, local_models=[self.model2.state_dict()], epoch=0)
        ...
        #         self.visualize_umap(global_model=self.model, local_models=local_models, epoch=epoch)
            ...
            #     if self.args.dist.freq > 0 and epoch % self.args.dist.freq == 0:
            #         self.evaluate_dist(model=self.model, prev_model_weight=prev_model_weight, local_models=local_models, local_datasets=local_datasets, epoch=epoch)

            #     if self.args.deviate.freq > 0 and epoch % self.args.collapse.freq == 0:
            #         pass
            #         self.evaluate_gradient_deviation_from_optimal(prev_model_weight=prev_model_weight, current_model=copy.deepcopy(self.model), epoch=epoch)
    ...
    def load_model(self, mode=1) -> None:
        if self.args.get('load_model_path') and mode==1:
        ...
        if self.args.get('load_model_path2') and mode==2:
            saved_dict = torch.load(self.args.load_model_path2)
            self.model2.load_state_dict(saved_dict['model_state_dict'], strict=False)
            if 'epoch' in saved_dict:
                self.start_round = saved_dict["epoch"]+1
                logger.warning(f'Load model from {self.args.load_model_path2}, epoch {saved_dict["epoch"]}')
            else:
                logger.warning(f'Load model from {self.args.load_model_path2}')
    ...
    def evaluate_dist(self, model: nn.Module, prev_model_weight: nn.Module, local_models: List[nn.Module],
                                   local_datasets: List[torch.utils.data.Dataset], epoch: int):
        results = self.evaler.evaluate_dist(model, prev_model_weight, local_models, local_datasets)
        self.wandb_log(results, step=epoch)


    def evaluate_gradient_deviation_from_optimal(self, prev_model_weight,  current_model: nn.Module, epoch: int, ):
        results = self.evaler.gd(prev_model_weight, current_model)
        self.wandb_log(results, step=epoch)
