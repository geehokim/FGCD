...
def get_dataset(args, trainset, mode='iid'):
    ...
    if dataset_name in ['cifar10', 'cifar100', 'cub']:
        ...
        filepath = directory + '/' + mode +  (
            str(args.split.alpha) if mode == 'dirichlet' else '') +  '_clients' + str(args.trainer.num_clients) + f'_{num_seen_classes}seen' + '.pt'
    ...
    if create_new_client_data == False:
        ...
        # elif mode[:4] == 'skew' and mode[-5:] == 'class':
            ...
            dataset = cifar_toyset(trainset, args.trainer.num_clients,
                                   num_valid_classes=args.split.limit_total_classes,
                                   limit_number_per_class=args.split.limit_number_per_class,
                                   toy_noniid_rate=args.split.toy_noniid_rate, non_iid=True)
        elif mode == 'toy_iid':
            dataset = cifar_toyset(trainset, args.trainer.num_clients,
                                   num_valid_classes=args.split.limit_total_classes,
                                   limit_number_per_class=args.split.limit_number_per_class,
                                   toy_noniid_rate=args.split.toy_noniid_rate, non_iid=False)
        else:
            assert False
        ...
        for client_idx, client_dataset_idxs in dataset.items():
            ...
            elif args.dataset.name in ['cub']:
                client = {
                    'idxs': client_dataset_idxs,
                    'class_dict': None
                }
                train_dataset_labelled, train_dataset_unlabelled = get_cub_datasets(args, trainset, client)
            else:
                assert False
