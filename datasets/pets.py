...
class OxfordIIITPet(Dataset):
    ...
    def update_targets(self):
        self.targets = [t for i, (p, t) in enumerate(self.samples)]
...
def get_pets_datasets_whole(root, train_transform, test_transform, train_classes=range(19), prop_train_labels=0.8,
                    split_train_val=False, seed=0):

    np.random.seed(seed)
    # Init entire training set
    whole_training_set = OxfordIIITPet(root=root, transform=train_transform, split='trainval', download=True)  # len = 3680

    # Get test set for all classes
    test_dataset = OxfordIIITPet(root=root, transform=test_transform, split='test', download=True)
...
def get_pets_datasets(args, trainset, client):
...
def main():
    x = get_pets_datasets_whole(None, None, split_train_val=False)
