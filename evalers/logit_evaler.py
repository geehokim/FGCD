...
class LogitEvaler():
    ...
    def local_trainset_eval(self, model, epoch, local_train_loader, gt_prior=None):
        model.eval()
        # model_device = next(model.parameters()).device
        model.to(self.device)
        test_transform = copy.deepcopy(self.test_loader.dataset.transform)
        labelled_dataset = copy.deepcopy(local_train_loader.dataset.labelled_dataset)
        labelled_dataset.transform = test_transform
        unlabelled_dataset = copy.deepcopy(local_train_loader.dataset.unlabelled_dataset)
        unlabelled_dataset.transform = test_transform

        labelled_loader = DataLoader(labelled_dataset, batch_size=self.args.batch_size, shuffle=False, num_workers=self.args.num_workers)


        unlabelled_loader = DataLoader(unlabelled_dataset, batch_size=self.args.batch_size, shuffle=False, num_workers=self.args.num_workers)
        ...
        # First extract unlabelled features
        ...
        # all_acc, old_acc, new_acc, w,  ind_map = self.log_accs_from_preds(y_true=targets, y_pred=preds, mask=mask,
        #                                                      T=epoch, eval_funcs=['v2'], save_name='Test Acc')

        
        total_feats = all_feats.clone().numpy()
        num_classes = len(np.unique(targets))
        # c, num_clust, req_c = FINCH(total_feats, req_clust=num_classes, distance='cosine', verbose=False)
        kmeans = KMeans(n_clusters=num_classes, random_state=0).fit(total_feats)
        req_c = kmeans.labels_

        #targets = np.concatenate(targets)
        all_acc_cluster, old_acc_cluster, new_acc_cluster, w_cluster, _ = self.log_accs_from_preds(y_true=targets, y_pred=req_c, mask=mask,
                                                        T=epoch, eval_funcs=['v2'], save_name='Test Acc')
        ...
        results = {
            ...
            'old_p_acc': 0,
            'new_p_acc': 0,
            'conf_matrix': w_cluster,
            'all_feats': all_feats,
            'targets': targets,
            'ind_map': None,
            'cluster_acc': all_acc_cluster * 100,
            'cluster_old_acc': old_acc_cluster * 100,
            'cluster_new_acc': new_acc_cluster * 100,
        }
    ...
    @torch.no_grad()
    # Setting test loaders for evaluating local loss
    def local_trainset_eval_unlabelled(self, model, epoch, local_train_loader, gt_prior=None):
        model.eval()
        # model_device = next(model.parameters()).device
        model.to(self.device)
        test_transform = copy.deepcopy(self.test_loader.dataset.transform)
        ...
        print('Collating logits for unlabelled data..')
        preds = []
        # First extract unlabelled features
        for batch_idx, (images, label, _) in enumerate(unlabelled_loader):
            images = images.to(self.device)
    ...
    @torch.no_grad()
    # Setting test loaders for evaluating local loss
    def local_trainset_eval_unlabelled_logit(self, model, epoch, local_train_loader, gt_prior=None):
        model.eval()
        # model_device = next(model.parameters()).device
        model.to(self.device)
        test_transform = copy.deepcopy(self.test_loader.dataset.transform)
    ...
    # Setting test loaders for evaluating local loss
    @torch.no_grad()
    def local_trainset_cluster_eval(self, model, epoch, local_train_loader, gt_prior=None):
        model.eval()
        model.to(self.device)
        test_transform = copy.deepcopy(self.test_loader.dataset.transform)
        labelled_dataset = copy.deepcopy(local_train_loader.dataset.labelled_dataset)
        ...
        c, num_clust, req_c = FINCH(all_feats.cpu().numpy(), req_clust=num_classes, distance='cosine', verbose=False)
    ...
    # def divergence_from_gt_prior(self, all_preds, gt_prior, ind_map):
    #     ind_map_preds_to_gt = {j: i for i, j in ind_map.items()}
    #     aligned_preds = []
    #     for pred in all_preds:
    #         aligned_preds.append(ind_map_preds_to_gt[pred])
    #
    #     ## Get frequency
    #     num_classes = len(gt_prior)
    #     prior = torch.zeros(num_classes)
    #     epsilon = torch.ones(num_classes) * 1e-7
    #     prior += epsilon
    #
    ...
    #             smooth_values[pred] = smooth_max
    #             prior += smooth_values
    #     estimated_prior = prior / len(aligned_preds)
    #     estimated_prior = torch.clamp(estimated_prior, min=1e-7)
    #
    #     # kl divergence
    #     KL = F.kl_div(estimated_prior.log(), gt_prior)
    #
    #     # MSE
    #     preds_entropy = torch.log(estimated_prior ** (-estimated_prior)) + math.log(float(len(estimated_prior)))
    #     target_entropy = torch.log(gt_prior ** (-gt_prior)) + math.log(float(len(gt_prior)))
    #     MSE = torch.sum((target_entropy - preds_entropy) ** 2)
    #
    #     return KL, MSE

    
    @torch.no_grad()
    def log_energy_scores(self, model, train_loader, seen_classes, device, temperature=1):
        model.eval()
        model.to(device)
        ...
        for images, labels, _, mask_lab in tqdm(train_loader, desc="Calculating energy scores"):
            ...
            # seen과 unseen 클래스 구분
            seen_mask = torch.tensor([label.item() in seen_classes for label in labels])
            seen_energies.extend(energy[seen_mask].cpu().tolist())
            unseen_energies.extend(energy[~seen_mask].cpu().tolist())

        # 통계 계산
        seen_mean = np.mean(seen_energies)
        seen_std = np.std(seen_energies)
        unseen_mean = np.mean(unseen_energies)
        unseen_std = np.std(unseen_energies)

        # wandb에 로깅
        wandb.log({
            "energy/seen_mean": seen_mean,
            "energy/seen_std": seen_std,
            "energy/unseen_mean": unseen_mean,
            "energy/unseen_std": unseen_std,
        })

        # 히스토그램 생성 및 로깅
        fig, ax = plt.subplots()
        ax.hist(seen_energies, bins=50, alpha=0.5, label='Seen Classes')
        ax.hist(unseen_energies, bins=50, alpha=0.5, label='Unseen Classes')
        ax.set_xlabel('Energy')
        ax.set_ylabel('Frequency')
        ax.set_title('Energy Distribution: Seen vs Unseen Classes')
        ax.legend()

        wandb.log({"energy/distribution": wandb.Image(fig)})
        plt.close(fig)

        return seen_energies, unseen_energies
    ...
    def split_cluster_acc_v1(self, y_true, y_pred, mask):
        ...
        return total_acc, old_acc, new_acc, None, None
    ...
    def split_cluster_acc_v3(self, y_true, y_pred, mask):
        """
        Calculate clustering accuracy. Require scikit-learn installed
        First compute linear assignment on all data, then look at how good the accuracy is on subsets
    ...
    def log_accs_from_preds(self, y_true, y_pred, mask, eval_funcs: List[str], save_name: str, T: int = None,
        ...
        for i, f_name in enumerate(eval_funcs):
            ...
            if f_name == 'v1':
                ...
                all_acc, old_acc, new_acc, w, ind_map = self.split_cluster_acc_v3(y_true, y_pred, mask)
    ...
    @torch.no_grad()
    def eval(self, model: nn.Module, epoch: int, device: torch.device = None, cluster_eval='feats', **kwargs) -> Dict:
        ...
        for batch_idx, (images, label, _) in enumerate(self.test_loader):
            images = images.to(self.device)
        ...
        all_acc, old_acc, new_acc, w, _ = self.log_accs_from_preds(y_true=targets, y_pred=preds, mask=mask,
                                                        T=epoch, eval_funcs=['v2'], save_name='Test Acc')
        ...
        if cluster_eval == 'feats':
            total_feats = all_feats.clone().numpy()
            # c, num_clust, req_c = FINCH(total_feats, req_clust=num_classes, distance='cosine', verbose=False)
            kmeans = KMeans(n_clusters=num_classes, random_state=0).fit(total_feats)
            req_c = kmeans.labels_
        elif cluster_eval == 'feats_proj':
            total_feats = all_p_feats.clone().numpy()
            kmeans = KMeans(n_clusters=num_classes, random_state=0).fit(total_feats)
            req_c = kmeans.labels_
    ...
    @torch.no_grad()
    def eval_cluster(self, model: nn.Module, epoch: int, device: torch.device = None, **kwargs) -> Dict:
        ...
        c, num_clust, req_c = FINCH(all_feats, req_clust=num_classes, distance='cosine', verbose=False, labeled_features=None)
    ...
    def log_divergence_from_gt_prior_global(self, local_est_priors, local_gt_priors):
        avg_local_est_prior = torch.stack(local_est_priors).mean(0)
        avg_local_gt_prior = torch.stack(local_gt_priors).mean(0)

        avg_local_est_prior =  avg_local_est_prior.cpu()
        avb_local_gt_prior = avg_local_gt_prior.cpu()

        # kl divergence
        KL = F.kl_div(avg_local_est_prior.log(), avb_local_gt_prior, reduction='batchmean')

        # MSE
        preds_entropy = torch.log(avg_local_est_prior ** (-avg_local_est_prior)) + math.log(float(len(avg_local_est_prior)))
        target_entropy = torch.log(avb_local_gt_prior ** (-avb_local_gt_prior)) + math.log(float(len(avb_local_gt_prior)))
        MSE = torch.sum((target_entropy - preds_entropy) ** 2)

        result = {
            f'{self.args.dataset.name}/global/MSE': MSE,
            f'{self.args.dataset.name}/global/KL': KL,
        }
        return result

    @torch.no_grad()
    def extract_features(self, model: nn.Module,  loader=None, return_feats_proj=False, **kwargs) -> Dict:
        ...
        # First extract all features
        for batch_idx, (images, label, _) in enumerate(dataset_loader):
            images = images.to(self.device)

            # Pass features through base model and then additional learnable transform (linear layer)
            features, feats_proj, logits = model(images, return_all=True)

            if return_feats_proj:
                all_feats.append(feats_proj.cpu())
            else:
    ...
    def plot_confusion_matrix(self, confusion_matrix, title='Confusion Matrix'):
        wandb_dict = {}
        fig_size = self.fig_size[self.args.dataset.name]
    ...
    def visualize_local_umap(self, model, client_idx, all_feats: torch.Tensor, targets, epoch: int):
        ...
        ## PLt setting
        if max(draw_classes) >= 10:
            cmap = plt.cm.get_cmap('gist_rainbow')  # 'jet', 'rainbow' 등으로 변경 가능
            color_cycle = [cmap(i / (max(draw_classes) - 1)) for i in range(max(draw_classes)+1)]
        ...
        for i in draw_classes:
            ...
            for j in range(len(drawing_option)):
                if drawing_option[j]:
                    ...
                    plt.scatter(this_embedding[:, 0], this_embedding[:, 1], color=color_cycle[i],
                                marker=marker_list[count], alpha=0.15)
                    plt.scatter(embedding[feat_len + j * num_total_classes + i, 0], embedding[feat_len + j * num_total_classes + i, 1],
                                color=color_cycle[i], marker="X", s=72,
                                alpha=1, )  # opacity_max - this_opacity_gap*count) #, label=classes[i] if first else None
                    plt.xticks([])  # Remove x-axis ticks
                    plt.yticks([])  # Remove y-axis ticks
    ...
    @torch.no_grad()
    def visualize_classifier_umap(self, global_model: nn.Module, local_models: List[nn.Module], local_datasets: List[torch.utils.data.Dataset], local_ind_map_pred_to_gt_list: List, epoch: int):

        wandb_dict = {}
        local_classifier_weights = []
        for local_model in local_models:
            local_classifier_weights.append(local_model['proj_layer.last_layer.parametrizations.weight.original1'].cpu().clone())
        local_classifier_weights = torch.cat(local_classifier_weights, dim=0)
        global_classifier_weights = global_model.proj_layer.last_layer.parametrizations.weight.original1.data.clone().cpu()
        classifier_weights = torch.cat((global_classifier_weights, local_classifier_weights), dim=0)

        ## Visualize UMAP
        reducer = umap.UMAP( n_components=2, metric='cosine', n_neighbors=10, min_dist = 0.5)
        embedding = reducer.fit_transform((classifier_weights.cpu().numpy()))

        num_classes = global_classifier_weights.size(0)
        global_centroids = embedding[:num_classes]
        local_centroids = embedding[num_classes:]
        
        color_cycle = [u'#1f77b4', u'#ff7f0e', u'#2ca02c', u'#d62728', u'#9467bd', u'#8c564b', u'#e377c2', u'#7f7f7f', u'#bcbd22', u'#17becf']
        custom_cmap = LinearSegmentedColormap.from_list('custom_cmap', color_cycle, N=10)
        marker_list = ['o','*','X','1', 'D', 'P']

        targets = torch.arange(num_classes)
        targets_local = torch.cat([targets for i in range(len(local_models))])

        f, ax = plt.subplots(figsize=(10, 10))

        # local centroids
        for i in range(self.args.trainer.num_clients):
            scatter = ax.scatter(local_centroids[i*num_classes:(i+1)*num_classes, 0], local_centroids[i*num_classes:(i+1)*num_classes, 1], c=targets.cpu().numpy(), cmap=custom_cmap, alpha=0.6, marker=marker_list[i], vmin=0, vmax=9)
        # global centroids
        scatter = ax.scatter(global_centroids[:, 0], global_centroids[:, 1], c=targets.cpu().numpy(), cmap=custom_cmap, alpha=1, marker=marker_list[5], vmin=0, vmax=9)

        #scatter = ax.scatter(embedding[:, 0], embedding[:, 1], c=targets_all.cpu().numpy(), cmap=custom_cmap, alpha=0.1, marker=marker_list[0])
        ...
        cb = plt.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax, ticks=np.arange(draw_classes) + 0.5)
        cb.set_ticklabels([f'Class {i}' for i in range(draw_classes)])
        fig_name = 'umap_server_classifiers'
        wandb_dict[fig_name] = wandb.Image(plt)  # filedir +args.set + args.mode + args.additional_experiment_name+this_name+'.png')
        os.makedirs("umap", exist_ok=True)
        pp = PdfPages(f"umap/{self.args.exp_name + fig_name}.pdf")
        pp.savefig(f, bbox_inches='tight', pad_inches=0.0)
    ...
    @torch.no_grad()
    def visualize_umaps(self, global_model: nn.Module, local_models: List[nn.Module], local_datasets: List[torch.utils.data.Dataset], local_ind_map_pred_to_gt_list: List, epoch: int):
        ...
        names_list = ["global", "local0", "local1", "local2", "local3", "local4"]
        #names_list = ["global","local0"]
        # names_list = ["global", "local0"]

        local_model0 = copy.deepcopy(global_model)
        local_model0.load_state_dict(local_models[0], strict=False)
        local_model1 = copy.deepcopy(global_model)
        local_model1.load_state_dict(local_models[1], strict=False)
        local_model2 = copy.deepcopy(global_model)
        local_model2.load_state_dict(local_models[2], strict=False)
        local_model3 = copy.deepcopy(global_model)
        local_model3.load_state_dict(local_models[3], strict=False)
        local_model4 = copy.deepcopy(global_model)
        local_model4.load_state_dict(local_models[4], strict=False)
        
        # models_list = [global_model] + [local_model0] + [local_model1]
        models_list = [global_model, local_model0, local_model1, local_model2, local_model3, local_model4]
        # models_list = [global_model, ]  # + [local_model0]
        # models_list = [global_model, local_model0]
        #models_list = [global_model]
        eval_result_dict = {}
        for name in names_list:
            eval_result_dict[name] = {}
            eval_result_dict[name]['saved_features'] = {}
        ...
        for name, model in zip(names_list, models_list):
            this_features, this_labels = self.extract_features(model=model, loader=test_loader, return_feats_proj=False)
            ...
            proxy = model.state_dict()['proj_layer.last_layer.parametrizations.weight.original1']
            normalized_proxy = torch.nn.functional.normalize(proxy, dim=1)
            eval_result_dict[name]['saved_proxies'] = normalized_proxy
        ...
        # ## Append proxies
        # feat_len = all_features.size(0)
        # for name in names_list:
        #     if 'local' in name and local_ind_map_pred_to_gt_list[int(name[-1])] is not None:
        #         ind_map = local_ind_map_pred_to_gt_list[int(name[-1])]
        #         full_map = {i: ind_map.get(i, i) for i in range(num_total_classes)}
        #         all_features = torch.cat((all_features, eval_result_dict[name]['saved_proxies'][list(full_map.values())]))
        #     else:
        #         all_features = torch.cat((all_features, eval_result_dict[name]['saved_proxies']))

        ## Reduce embedding using UMAP
        reducer = umap.UMAP( n_components=umap_args.umap_dim, metric='cosine', n_neighbors=10,
                            min_dist=0.5)
                ...
                for j in range(len(drawing_option)):
                    if drawing_option[j]:
                        ...
                        if umap_args.umap_dim == 2:
                            ax.scatter(this_embedding[:,0],this_embedding[:,1], color = color_cycle[i], marker =marker_list[count], alpha = 0.2)#opacity_max - this_opacity_gap*count) #, label=classes[i] if first else None
                            # ax.scatter(embedding[feat_len + j * draw_classes + i, 0],
                            #             embedding[feat_len + j * draw_classes + i, 1],
                            #             color=color_cycle[i], marker=marker_list[count], s=72,
                            #             alpha=1, )
                        elif umap_args.umap_dim == 3:
                            ax.scatter(this_embedding[:, 0], this_embedding[:, 1], this_embedding[:, 2], color=color_cycle[i],
                                       marker=marker_list[count],
                                       alpha=0.2)  # opacity_max - this_opacity_gap*count) #, label=classes[i] if first else None
                            ax.scatter(embedding[feat_len + j * draw_classes + i, 0],
                                       embedding[feat_len + j * draw_classes + i, 1],
                                       embedding[feat_len + j * draw_classes + i, 2],
                                       color=color_cycle[i], marker=marker_list[count], s=72,
                                       alpha=1, )
                        # plt.xticks([])  # Remove x-axis ticks
                        # plt.yticks([])  # Remove y-axis ticks
