...
from datasets.data_utils import DatasetSplit, get_dataset
...
class LogitEvaler():
    ...
    def local_trainset_eval(self, model, epoch, local_train_loader):
        ...
        all_acc, old_acc, new_acc, _ = self.log_accs_from_preds(y_true=targets, y_pred=preds, mask=mask,
    ...
    def log_accs_from_preds(self, y_true, y_pred, mask, eval_funcs: List[str], save_name: str, T: int = None,
        ...
        for i, f_name in enumerate(eval_funcs):
            ...
            if f_name == 'v2':
                all_acc, old_acc, new_acc, w = self.split_cluster_acc_v2(y_true, y_pred, mask)
            log_name = f'{save_name}_{f_name}'

            if i == 0:
                to_return = (all_acc, old_acc, new_acc, w)
    ...
    @torch.no_grad()
    def extract_features(self, model: nn.Module, epoch: int, device: torch.device = None, **kwargs) -> Dict:
    ...
    #----- UMAP ----#
    @torch.no_grad()
    def visualize_server_umap(self, all_feats: torch.Tensor, epoch: int):
        pass
    @torch.no_grad()
