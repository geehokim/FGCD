...
class ClusterEvaler():
    ...
    def local_trainset_eval_unlabelled(self, model, epoch, local_train_loader):
        model.eval()
        ...
        for batch_idx, (images, label, _) in enumerate(tqdm.tqdm(unlabelled_loader)):
            images = images.to(self.device)

            # Pass features through base model and then additional learnable transform (linear layer)
            feats, feats_proj, _, _, _ = model(images, return_all=True)
        ...
        cluster_feature = all_feats.cpu().numpy()
        idx2label, idx_single, dists = cluster_by_infomap(cluster_feature, k=self.args.client.top_k, tao_f=self.args.client.tao_f, dataset_name=self.args.dataset.name, silent=True)
        ...
        all_acc, old_acc, new_acc, _, _ = self.log_accs_from_preds(
            y_true=targets,
        ...
        results = {
            ...
            'infomap_acc': all_acc * 100,
            'infomap_old_acc': old_acc * 100,
            'infomap_new_acc': new_acc * 100,
        }
        return results
    ...
    def eval(self, model: nn.Module, epoch: int, device: torch.device = None, **kwargs) -> Dict:
        ...
        for batch_idx, (images, label, _) in enumerate(tqdm.tqdm(self.test_loader)):
            ...
            # Pass features through base model and then additional learnable transform (linear layer)
            feats, feats_proj, logits_labelled, logits_clustered, _ = model(images, return_all=True)
        ...
        if self.args.evaler.alg == 'kmeans':
                kmeans = KMeans(n_clusters=num_classes, random_state=0).fit(all_feats.numpy())
                req_c = kmeans.labels_
        else:
